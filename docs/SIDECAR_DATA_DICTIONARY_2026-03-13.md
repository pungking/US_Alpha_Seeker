...
- `entryFeasible=false` 또는 `tradePlanStatusShadow != VALID_EXEC` 인 경우,
  기존 risk-off 계열 평결이 아닌 한 `executionVerdict=HOLD_WAIT_PRICE`로 실행 타이밍만 표시한다.
- `aiVerdict/finalVerdict/verdict/verdictFinal`은 알파 방향성의 canonical verdict로 보존한다.
  실행 가능 여부는 `finalDecision/executionBucket/decisionReason`을 기준으로 판단한다.
...
| `executionVerdict` | 실행 타이밍 보조 평결 | 예: `HOLD_WAIT_PRICE`; 알파 verdict를 덮어쓰지 않음 |
...
| `price` | Stage6 산출 시점 현재가 | 감사/sidecar 진입거리 재검증용 |
...
| `idempotencyTelemetry` | object | 원장 중복 카운트와 skip 기반 중복 카운트를 분리한 표시용 진단(`ledgerDuplicateCount/skipDuplicateCount/effectiveDuplicateCount`) |
...
| `openOrderMonitor` | object | 기존 open buy entry의 체결성/재가격화 관측 요약(`open/matched/reprice/cancel/reasonCounts/records`) |
| `openOrderMonitorDetail` | string | 텔레그램/감사용 open order 상세 요약(`limit/current/suggested/delta/distance/rr/age/reason`) |
| `openOrderMonitorReprice` | object | monitor 기반 reprice bridge 적용 결과(`enabled/applied/blocked/reasonCounts`) |
| `stage6BlockerSamples` | array | Stage6 실행 후보 0건/무주문 상황에서 상위 차단 심볼과 `finalDecision/decisionReason/skipHint` 샘플 |
...
| `openOrderMonitor` | 기존 open buy entry의 체결성/재가격화 관측 판단(`status/reason/ageMinutes/limitPrice/currentPrice/distancePct/rrAtLimit/rrAtCurrent/suggestedLimitPrice`) |
...
| `NO_TRADE` | Stage6 limit 자체의 RR이 부족하거나, 근접 추격 진입 시 목표가 여유가 부족해 진입 비권장 |
| `DATA_MISSING` | Alpaca data API 응답 부족/실패로 현재가 기반 판단 불가 |

주의:
- v1은 observe-only다. 이 값은 감사/판단 근거이며 payload 가격·손절·타겟을 직접 변경하지 않는다.
- 현재가/추격가 기준 RR이 낮아도 Stage6 limit 기준 RR이 유효하면 `NO_TRADE`가 아니라 `PULLBACK_LIMIT`로 유지한다.
  이 경우 `near_entry_chase_rr_below_floor_keep_limit` 같은 reason으로 추격 금지/눌림 대기를 구분한다.
...
| 값 | 의미 |
|---|---|
| `KEEP` | 현재가가 limit 근처이거나 limit 이하라 기존 주문 유지가 합리적 |
| `WATCH_PULLBACK` | 주문은 유효하지만 현재가가 limit보다 높아 눌림 대기 필요 |
| `REPRICE_CANDIDATE` | 주문이 충분히 오래 열려 있고 RR/타겟 버퍼를 보존하는 상향 limit 후보가 있음 |
| `CANCEL_CANDIDATE` | 중복 open entry, limit 기준 RR 부족, 타겟 버퍼 부족, 장시간 괴리 등으로 취소 후보 |
| `DATA_MISSING` | open order 또는 현재가 데이터 부족 |
| `NO_OPEN_ORDER` | 해당 심볼의 open buy entry 없음 |

주의:
- v1은 observe-only다. 이 값은 주문 취소/재가격화 후보를 표시할 뿐 실제 cancel/replace를 수행하지 않는다.
- 실제 stale cancel/replace는 `ENTRY_OPEN_ORDER_STALE_CANCEL_ENABLED=true`로 별도 활성화해야 한다.
- `suggestedLimitPrice`는 Stage6 target/stop을 기준으로 최소 RR과 target buffer를 보존하는 상한이다. 자동 주문 변경 가격이 아니라
  수동 검토/차후 promote 기준이다.
- `idempotencyTelemetry.effectiveDuplicateCount`는 표시용 카운터다. 내부 dedupe 원장 값(`idempotency.duplicateCount`)을 변경하지 않고,
  `skipped[].reason=idempotency_duplicate`와 함께 읽어야 한다.
...
Stage6 후보 전체에 대해 주문 제출 이후 체결성 상태를 결합한 스냅샷.

입력:
- `state/last-dry-exec-preview.json`
- `state/last-order-decision-audit.json`
- Alpaca paper open/closed orders 및 FILL activity(자격증명 없으면 broker unavailable로 기록)

| 필드 | 타입 | 의미 |
|---|---|---|
| `generatedAt` | string | 리포트 생성 시각 |
| `source.previewFound` | boolean | dry-run preview 존재 여부 |
| `source.decisionAuditFound` | boolean | order decision audit 존재 여부 |
| `broker.available` | boolean | Alpaca 조회 가능 여부 |
| `broker.lookbackHours` | number | closed/fill 조회 lookback |
| `summary.overall` | string | `pass/warn` |
| `summary.candidateCount` | number | 후보 행 수 |
| `summary.payloadCount` | number | payload 수 |
| `summary.skippedCount` | number | skip 수 |
| `summary.brokerAttempted` | number | 브로커 제출 시도 |
| `summary.brokerSubmitted` | number | 브로커 제출 성공 |
| `summary.openOrderCount` | number | 현재 open order 수 |
| `summary.fillActivityCount` | number | lookback 내 fill activity 수 |
| `summary.openRepricedWaiting` | number | 이미 reprice된 뒤 pullback 대기 중인 open order 수 |
| `summary.openReprice` | number | open reprice 후보 수 |
| `summary.openCancel` | number | open cancel 후보 수 |
| `summary.entryTooFar` | number | entry distance로 차단된 후보 수 |
| `summary.highPriceSizeBlocked` | number | 1회 notional이 limit price보다 작거나 min-one-share cap/risk에 막힌 후보 수 |
| `summary.avgOpenCurrentVsLimitPct` | number | 현재가와 open limit 평균 괴리(+) |
| `rows[]` | array | 후보별 fillability 진단 |

`rows[]` 핵심 필드:
- `symbol`, `status`, `reason`
- `entryAdjusted`, `target`, `stop`, `currentPrice`, `activeLimit`
- `currentVsLimitPct`, `rrAtAdjustedEntry`, `rrAtCurrent`
- `oneShareNotional`, `oneShareRiskDollars`
- `monitorStatus`, `monitorSuggestedLimit`, `monitorSuggestedDeltaPct`
- `brokerOpenStatus`, `brokerClosedStatus`, `fillCount`, `fillQty`, `avgFillPrice`

운영 해석:
- `OPEN_WAITING`: 주문은 살아 있으나 아직 미체결.
- `OPEN_REPRICED_WAITING`: reprice가 이미 적용된 open 주문이며 pullback/체결 대기.
- `OPEN_REPRICE_CANDIDATE`: monitor가 RR-safe stale reprice 가능성을 제시.
- `OPEN_CANCEL_CANDIDATE`: monitor가 RR 저하/중복/target-buffer 부족 등으로 cancel 후보를 제시.
- `BLOCKED_ENTRY_DISTANCE`: Stage6/entry-price 조정 후에도 현재가와 너무 멀어 payload 제외.
- `BLOCKED_HIGH_PRICE_SIZE`: 고가 종목에서 기본 notional로 1주 주문이 불가능하거나, min-one-share cap/risk 조건을 통과하지 못해 payload 제외.
- `TERMINAL_UNFILLED`: 취소/만료/거절로 종료됐고 fill 없음.
- `FILLED`: Alpaca fill activity 또는 broker filled 상태 확인.

## `state/execution-blocker-audit.json`

여러 sidecar dry-run artifact 또는 state 디렉터리를 재귀 스캔해 "workflow success"와 "실제 실행 가능"을 분리한다.

| 필드 | 타입 | 설명 |
|---|---|---|
| `summary.runCount` | number | 분석한 dry-run state 수 |
| `summary.uniqueStage6Count` | number | 고유 Stage6 hash 수 |
| `summary.brokerSubmittedRuns` | number | broker 제출 성공 run 수 |
| `summary.payloadReadyRuns` | number | payload가 1개 이상 생성된 run 수 |
| `summary.zeroExecutableRuns` | number | Stage6 실행 후보가 0개인 run 수 |
| `summary.candidateBlockedRuns` | number | sidecar 후보는 있었지만 sizing/distance 등으로 차단된 run 수 |
| `summary.classifications` | object | `STAGE6_ZERO_EXECUTABLE`, `SIDECAR_CANDIDATE_BLOCKED` 등 분류별 카운트 |
| `summary.stageReasons` | object | Stage6 계약 차단 사유 집계 |
| `summary.skipReasons` | object | sidecar payload 차단 사유 집계 |
| `stage6Groups[].blockerSamples` | array | Stage6 hash별 대표 차단 심볼 샘플 |
| `stage6Groups[]` | array | Stage6 hash별 대표 원인 요약 |
| `runs[]` | array | 개별 run 진단 행 |

운영 해석:
- `STAGE6_ZERO_EXECUTABLE`: Alpaca/sidecar 문제가 아니라 Stage6 최종 게이트가 실행 후보를 모두 제거.
- `SIDECAR_CANDIDATE_BLOCKED`: Stage6 후보는 있었으나 entry distance, 고가 sizing, RR 등 sidecar 실행 정책에서 차단.
- `PAYLOAD_READY_NOT_SUBMITTED`: payload는 생성됐지만 preflight/dedupe/submit policy 때문에 제출되지 않음.
- `BROKER_SUBMITTED`: Alpaca 제출 경로는 통과했으며 이후 fillability/reprice 분석 대상.
