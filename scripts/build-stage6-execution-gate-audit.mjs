  ...
  for (const [group, list] of groups) {
    ...
    for (const row of list) {
      ...
      out.push({
        ...
        zeroExecutableFormulaBlockedBy: normalizeReasonArray(row?.zeroExecutableFormulaBlockedBy ?? row?.zeroExecutableFormula?.blockedBy),
        zeroExecutableFormulaNextAction: normalizeText(row?.zeroExecutableFormulaNextAction ?? row?.zeroExecutableFormula?.nextAction),
        zeroExecutableFormulaDoneWhenEvidence: normalizeReasonArray(row?.zeroExecutableFormulaDoneWhenEvidence ?? row?.zeroExecutableFormula?.doneWhenEvidence),
...
function extractRowsFromStage6(filePath, payload, notionRows) {
  ...
  for (const [symbol, bundle] of bySymbol.entries()) {
    ...
    out.push({
      ...
      zeroExecutableFormulaBlockedBy: normalizeReasonArray(row?.zeroExecutableFormulaBlockedBy ?? notion?.zeroExecutableFormulaBlockedBy ?? row?.zeroExecutableFormula?.blockedBy),
      zeroExecutableFormulaNextAction: normalizeText(row?.zeroExecutableFormulaNextAction ?? notion?.zeroExecutableFormulaNextAction ?? row?.zeroExecutableFormula?.nextAction),
      zeroExecutableFormulaDoneWhenEvidence: normalizeReasonArray(row?.zeroExecutableFormulaDoneWhenEvidence ?? notion?.zeroExecutableFormulaDoneWhenEvidence ?? row?.zeroExecutableFormula?.doneWhenEvidence),
