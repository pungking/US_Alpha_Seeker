...
function formulaManifestContractIssues(stage6) {
  ...
  for (const [lane, expectedFields] of Object.entries(EXPECTED_FORMULA_CONTRACT.tunablePolicyFields)) {
    const actualFields = new Set(Array.isArray(tunablePolicyFields[lane]) ? tunablePolicyFields[lane] : []);
    if (expectedFields.length === 0 && !Array.isArray(tunablePolicyFields[lane])) {
      issues.push({ issue: 'formula_contract_tunable_policy_fields_missing', lane, field: 'empty_contract' });
    }
    for (const field of expectedFields) {
      if (!actualFields.has(field)) {
        issues.push({ issue: 'formula_contract_tunable_policy_field_missing', lane, field });
      }
    }
  }
  const promotionSafetyRules = new Set(Array.isArray(contract.promotionSafetyRules) ? contract.promotionSafetyRules : []);
  for (const rule of EXPECTED_FORMULA_CONTRACT.promotionSafetyRules) {
    if (!promotionSafetyRules.has(rule)) {
      issues.push({ issue: 'formula_contract_promotion_safety_rule_missing', rule });
    }
  }
  return issues;
}
...
function compactRow(row) {
  ...
  return {
    ...
    breakoutRetestProofReasons: Array.isArray(row?.breakoutRetestProofReasons) ? row.breakoutRetestProofReasons : [],
    ...
    breakoutRetestProofRetestCloseReclaimed: row?.breakoutRetestProofRetestCloseReclaimed ?? null,
    breakoutRetestProofUndercutReclaimFound: row?.breakoutRetestProofUndercutReclaimFound ?? null,
    ...
    targetRecalibrationViabilityReasons: Array.isArray(row?.targetRecalibrationViabilityReasons) ? row.targetRecalibrationViabilityReasons : [],
    targetRecalibrationRequiredTargetPrice: numberOrNull(row?.targetRecalibrationRequiredTargetPrice),
    targetRecalibrationCurrentTargetPrice: numberOrNull(row?.targetRecalibrationCurrentTargetPrice),
    ...
    targetRecalibrationRequiredTargetByExecutionFloorPrice: numberOrNull(row?.targetRecalibrationRequiredTargetByExecutionFloorPrice),
    ...
    targetRecalibrationRequiredTargetDominantReason: row?.targetRecalibrationRequiredTargetDominantReason || null,
    ...
    structurePolicyReasons: Array.isArray(row?.structurePolicyReasons) ? row.structurePolicyReasons : [],
    ...
    structurePolicyCurrentRrValue: numberOrNull(row?.structurePolicyCurrentRrValue),
    structurePolicyMinRr: numberOrNull(row?.structurePolicyMinRr),
    structurePolicyEntryDistancePct: numberOrNull(row?.structurePolicyEntryDistancePct),
    structurePolicyMaxReviewDistancePct: numberOrNull(row?.structurePolicyMaxReviewDistancePct),
