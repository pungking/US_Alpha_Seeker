...
function markdown(report) {
  ...
  const evidenceRows = (report.backlog?.producerEvidenceRows || [])
    .map((row) => {
      ...
      const decisionPackageSummary = `blockedBy=${(row.formulaBlockedBy || []).join(',') || 'none'} nextAction=${row.formulaNextAction || 'N/A'} doneWhen=${(row.formulaDoneWhenEvidence || []).join(',') || 'none'}`;
      return `| ${row.symbol || 'N/A'} | ${row.producerTrack || 'N/A'} | ${row.adjustmentKnob || 'N/A'} | ${row.rowEvidenceSummary || 'N/A'} | ${decisionPackageSummary} | ${targetSummary} | ${structureSummary} |`;
    })
  ...
  return `# Stage6 Runtime Formula Contract Proof\n\n` +
    ...
    `| Symbol | Track | Knob | Row Evidence | Decision Package | Target Evidence | Structure Evidence |\n| --- | --- | --- | --- | --- | --- | --- |\n${evidenceRows}\n\n` +
    `## Split Tuning Lanes\n\n` +
...
function main() {
  ...
  const producerEvidenceRows = (backlogReport.backlogRows || [])
    ...
    .map((row) => ({
      ...
      formulaBlockedBy: row.formulaBlockedBy || [],
      formulaNextAction: row.formulaNextAction || null,
      formulaDoneWhenEvidence: row.formulaDoneWhenEvidence || [],
