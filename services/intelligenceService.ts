...
const ALPHA_SCHEMA = {
  ...
  items: {
    ...
    properties: {
      ...
      // 차트 분석 필드 추가
      chartPattern: { type: Type.STRING, description: "Detected technical pattern (e.g. Fibonacci 0.618, Cup & Handle, Bull Flag)" },
      supportLevel: { type: Type.NUMBER, description: "Primary support/entry price level" },
      resistanceLevel: { type: Type.NUMBER, description: "Next major resistance/target price level" },
      riskRewardRatio: { type: Type.STRING, description: "Calculated Risk-to-Reward ratio (e.g. 1:3.5)" }
    },
    required: [
      "symbol", "aiVerdict", "marketCapClass", "sectorTheme", "investmentOutlook", 
      "selectionReasons", "convictionScore", "expectedReturn", "theme", 
      "aiSentiment", "analysisLogic", "chartPattern", "supportLevel", "resistanceLevel", "riskRewardRatio"
    ]
  }
};
...
이 중 시장 주도력이 가장 강력한 6개 종목을 최종 선정하여 기술적 타점이 포함된 정밀 보고서를 작성하세요.
반드시 아래 형식을 엄수하여 JSON 배열만 응답하십시오. 다른 설명이나 텍스트는 절대 금지합니다.

Technical Analysis Required:
- chartPattern: 피보나치 되돌림, 컵앤핸들, 불플래그, 헤드앤숄더 등 현재 가격대에서 형성된 구체적인 패턴 명시.
- supportLevel & resistanceLevel: 차트 기반의 실제 지지선과 저항선 수치(숫자).
- riskRewardRatio: 진입가 대비 목표 수익의 비율 계산 (예: 1:3).
    ...
    if (provider === ApiProvider.PERPLEXITY) {
      const res = await fetch('https://api.perplexity.ai/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'sonar-pro', 
          messages: [
            { role: "system", content: `당신은 ${today} 기준의 한국어 금융 분석 리포트를 작성하는 AI 에이전트입니다. 차트 패턴과 손익비를 반드시 포함하며, JSON 코드 블록 하나만 출력하십시오.` },
...
2. 6단계에서 분석된 차트 패턴과 손익비의 타당성을 시장 매크로 상황과 대조하여 비판적으로 검토하십시오.
