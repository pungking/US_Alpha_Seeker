...
async function fetchWithRetry(fn: () => Promise<any>, retries = 2, delay = 5000): Promise<any> {
  try {
    ...
    if (retries > 0 && isQuotaError) {
      console.warn(`[AI_RETRY] Quota hit. Waiting ${delay}ms before retry...`);
...
export async function generateAlphaSynthesis(candidates: any[], provider: ApiProvider): Promise<{data: any[] | null, error?: string}> {
  ...
  try {
    if (provider === ApiProvider.GEMINI) {
      ...
      try {
        ...
        if (geminiErr.message?.includes("429")) {
          return { data: null, error: "QUOTA_EXHAUSTED: Gemini API 할당량이 부족합니다. 약 1분 후 시도하거나 Sonar Pro 엔진으로 전환하십시오." };
        }
        throw geminiErr;
      }
    }
    ...
    if (provider === ApiProvider.PERPLEXITY) {
      ...
      if (!res.ok) return { data: null, error: `PERPLEXITY_ERR_${res.status}` };
...
export async function analyzePipelineStatus(data: any, provider: ApiProvider): Promise<string> {
  ...
  try {
    ...
    return `분석 지연 또는 오류: ${e.message.includes("429") ? "API 할당량 초과" : e.message}`;
  }
}
