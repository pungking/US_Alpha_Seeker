...
/**
 * AI의 응답 텍스트에서 순수 JSON 데이터를 추출하는 강화된 파서
 */
function sanitizeAndParseJson(text: string): any | null {
  ...
  try {
    ...
    console.error("JSON_PARSE_CRITICAL_FAILURE:", e, "Raw Text Preview:", text.substring(0, 100));
    return null;
  }
}
...
이 중 시장 주도력이 강력한 6개 종목을 최종 선정하여 정밀 분석 보고서를 작성하세요.
    ...
    if (provider === ApiProvider.PERPLEXITY) {
      const res = await fetch('https://api.perplexity.ai/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'sonar-pro',
          messages: [
            { role: "system", content: "백테스트 시뮬레이션 결과를 JSON 객체 하나로 응답하십시오." },
            { role: "user", content: prompt }
          ],
          temperature: 0.2
        })
