...
const PreliminaryFilter: React.FC = () => {
  ...
  const executeIntegratedPurification = async () => {
    if (!accessToken) {
    ...
    try {
      ...
      const payload = {
        manifest: { version: "1.7.0", node: "Integrated_Filter", count: filtered.length, timestamp: new Date().toISOString() },
            ...
            <div className="flex items-center space-x-6">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 mt-2">
                   ...
                   <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest italic">Stage 0 to Stage 1 Transition</span>
                </div>
              </div>
            </div>
            <button 
              onClick={executeIntegratedPurification} 
              disabled={loading}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div className="col-span-2 bg-gradient-to-br from-emerald-600/10 to-transparent p-8 rounded-3xl border border-emerald-500/10">
              ...
              <div className="flex items-baseline space-x-6">
                ...
                <span className="text-slate-500 text-[10px] uppercase font-bold tracking-widest italic">Pure Equities Detected</span>
              </div>
            </div>
            <div className="bg-black/20 p-8 rounded-3xl border border-white/5 flex flex-col justify-center text-center">
              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-2">Market Volatility Floor</p>
              <p className="text-3xl font-black text-indigo-500/80 italic">${minPrice.toFixed(2)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="xl:col-span-1">
        <div className="glass-panel h-[720px] rounded-[40px] bg-slate-950 border-l-4 border-l-emerald-600 flex flex-col p-6 shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between mb-8 px-2">
            ...
            <div className={`w-2 h-2 rounded-full ${loading ? 'bg-emerald-500 animate-pulse' : 'bg-slate-700'}`}></div>
          </div>
