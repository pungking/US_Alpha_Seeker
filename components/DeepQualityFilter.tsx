...
const DeepQualityFilter: React.FC<Props> = ({ onComplete, autoStart }) => {
  ...
  const [logs, setLogs] = useState<string[]>(() => {
    ...
    return cached ? JSON.parse(cached) : ['> Quality_Node v2.3.2: Protocol Handshake Initiated.'];
  });
  ...
  const findFileWithRetry = async (query: string, retries = 5): Promise<any> => {
    for (let i = 0; i < retries; i++) {
      ...
      addLog(`Indexing wait... (${i + 1}/${retries})`, "warn");
  ...
  const executeIntegratedScan = async () => {
    ...
    try {
      ...
      if (!file) {
        addLog("Stage 1 input missing. Flow suspended.", "err");
        setLoading(false);
        return;
      }

      addLog(`Matrix Found: ${file.name}. Pulling data...`, "ok");
      ...
      addLog(`Scanning Top ${limit} Global Assets for Financial Integrity...`, "info");
      ...
      for (let i = 0; i < limit; i++) {
        ...
        try {
          ...
          addLog(`Skipped node: ${target.symbol}`, "warn");
          await new Promise(r => setTimeout(r, 1500));
        }
      }

      setProcessedData(results);
      addLog(`Purification Complete: ${results.length} nodes saved.`, "ok");

      const folderId = await ensureFolder(accessToken, GOOGLE_DRIVE_TARGET.stage2SubFolder);
      const fileName = `STAGE2_ELITE_UNIVERSE_${new Date().toISOString().split('T')[0]}.json`;
      const payload = { manifest: { version: "2.3.2", count: results.length }, elite_universe: results };
        ...
        <div className="glass-panel p-8 md:p-10 rounded-[40px] border-t-2 border-t-purple-500 shadow-2xl bg-slate-900/40 relative overflow-hidden">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
            <div className="flex items-center space-x-6">
              ...
              <div>
                <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">Elite_Scanner v2.3.2</h2>
              </div>
            </div>
            <button onClick={executeIntegratedScan} disabled={loading} className="px-12 py-5 bg-purple-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 shadow-xl shadow-purple-900/20 transition-all">
              {loading ? 'Analyzing...' : 'Scan & Commit Vault'}
            </button>
          </div>
      ...
      <div className="xl:col-span-1">
        <div className="glass-panel h-[720px] rounded-[40px] bg-slate-950 border-l-4 border-l-purple-600 flex flex-col p-6 shadow-2xl overflow-hidden">
          ...
          <div ref={logRef} className="flex-1 bg-black/70 p-6 rounded-[32px] font-mono text-[9px] text-purple-300/60 overflow-y-auto no-scrollbar space-y-4 border border-white/5 leading-relaxed">
            {logs.map((l, i) => (<div key={i} className={`pl-4 border-l-2 ${l.includes('[OK]') ? 'border-emerald-500 text-emerald-400 bg-emerald-500/5' : 'border-purple-900'}`}>{l}</div>))}
          </div>
        </div>
      </div>
    </div>
