...
const DeepQualityFilter: React.FC<Props> = ({ onComplete, autoStart }) => {
  ...
  // 마운트 시 데이터 로드 및 오토파일럿 트리거
  useEffect(() => {
    if (accessToken) {
      const timer = setTimeout(() => {
        if (autoStart) {
          addLog("Auto-Pilot Sequence: Initializing Stage 2...", "info");
          executeIntegratedScan();
        } else {
          // 수동 전환 시에도 이전 데이터 로드 시도
          loadLastResults();
        }
      }, 1000);
  ...
  const findFileWithRetry = async (query: string, retries = 8, delay = 4000): Promise<any> => {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        addLog(`Vault indexing in progress... waiting 4s (${i + 1}/${retries})`, "warn");
  ...
  const loadLastResults = async () => {
    ...
    const file = await findFileWithRetry(q, 3, 2000);
    if (file) {
      addLog(`Existing Stage 2 data recovered: ${file.name}`, "ok");
      const content = await fetch(`https://www.googleapis.com/drive/v3/files/${file.id}?alt=media`, {
        headers: { 'Authorization': `Bearer ${accessToken}` }
      }).then(r => r.json());
      if (content.elite_universe) setProcessedData(content.elite_universe);
    }
  };
  ...
  const executeIntegratedScan = async () => {
    ...
    setLoading(true);
    addLog("Phase 1: Retrieving Purified Matrix from Stage 1...", "info");
    ...
    try {
      ...
      if (!file) {
        addLog("CRITICAL: Stage 1 data not found. Flow halted.", "err");
        setLoading(false);
        return;
      }

      addLog(`Matrix Detected: ${file.name}. Pulling data...`, "ok");
      const content = await fetch(`https://www.googleapis.com/drive/v3/files/${file.id}?alt=media`, {
        headers: { 'Authorization': `Bearer ${accessToken}` }
      }).then(r => r.json());
      ...
      addLog(`Analyzing TOP ${limit} assets. This will take approx 2-3 mins.`, "info");
      ...
      addLog(`Protocol Complete. Committing to Stage 2 Vault...`, "ok");

      const folderId = await ensureFolder(accessToken, GOOGLE_DRIVE_TARGET.stage2SubFolder);
      const fileName = `STAGE2_ELITE_UNIVERSE_${new Date().toISOString().split('T')[0]}.json`;
      const payload = { manifest: { version: "2.3.1", count: results.length }, elite_universe: results };
    ...
    <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 mt-2">
                   <span className="text-[8px] font-black px-2 py-0.5 rounded border border-purple-500/20 bg-purple-500/10 text-purple-400 uppercase tracking-widest italic animate-pulse">
                     {loading ? 'Synthesizing Data...' : 'Synchronization Active'}
                   </span>
                </div>
              </div>
            </div>
            <button onClick={executeIntegratedScan} disabled={loading} className="px-12 py-5 bg-purple-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl shadow-purple-900/20">
      ...
      <div className="xl:col-span-1">
        <div className="glass-panel h-[720px] rounded-[40px] bg-slate-950 border-l-4 border-l-purple-600 flex flex-col p-6 shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between mb-8 px-2"><h3 className="font-black text-white text-[10px] uppercase tracking-[0.4em] italic">Elite_Terminal</h3></div>
          <div ref={logRef} className="flex-1 bg-black/70 p-6 rounded-[32px] font-mono text-[9px] text-purple-300/60 overflow-y-auto no-scrollbar space-y-4 border border-white/5 leading-relaxed">
            {logs.map((l, i) => (<div key={i} className={`pl-4 border-l-2 ${l.includes('[OK]') ? 'border-emerald-500 text-emerald-400 bg-emerald-500/5' : l.includes('[ERR]') ? 'border-red-500 text-red-400 bg-red-500/5' : 'border-purple-900'}`}>{l}</div>))}
          </div>
        </div>
      </div>
    </div>
