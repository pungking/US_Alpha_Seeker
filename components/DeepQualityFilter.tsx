...
const DeepQualityFilter: React.FC = () => {
  ...
  const executeIntegratedScan = async () => {
    ...
    try {
      ...
      if (!listRes.files?.length) throw new Error("Stage 1 input not found.");
      ...
      const targetLimit = Math.min(equities.length, 500);
      setProgress({ current: 0, total: targetLimit });
      addLog(`Matrix Synced. Scanning Top ${targetLimit} Elite assets...`, "ok");
      ...
      const payload = {
        manifest: { version: "2.3.0", node: "Elite_Extraction", count: results.length, timestamp: new Date().toISOString() },
      ...
      <div className="xl:col-span-3 space-y-6">
        <div className="glass-panel p-8 md:p-10 rounded-[40px] border-t-2 border-t-purple-500 shadow-2xl bg-slate-900/40 relative overflow-hidden">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
            ...
            <button 
              onClick={executeIntegratedScan}
              disabled={loading}
              className={`px-12 py-5 bg-purple-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-purple-900/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50`}
            >
              {loading ? 'Analyzing & Uploading...' : 'Scan Top 500 & Commit'}
            </button>
          </div>
      ...
      <div className="xl:col-span-1">
        <div className="glass-panel h-[720px] rounded-[40px] bg-slate-950 border-l-4 border-l-purple-600 flex flex-col p-6 shadow-2xl overflow-hidden">
          ...
          <div ref={logRef} className="flex-1 bg-black/70 p-6 rounded-[32px] font-mono text-[9px] text-purple-300/60 overflow-y-auto no-scrollbar space-y-4 border border-white/5">
            {logs.map((l, i) => (
              <div key={i} className={`pl-4 border-l-2 ${l.includes('[OK]') ? 'border-emerald-500 text-emerald-400' : l.includes('[ERR]') ? 'border-red-500 text-red-400' : 'border-purple-900'}`}>
                {l}
              </div>
            ))}
          </div>
