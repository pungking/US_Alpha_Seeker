...
const TechnicalAnalysis: React.FC = () => {
  ...
  const executeIntegratedTechAudit = async () => {
    ...
    try {
      ...
      if (!listRes.files?.length) throw new Error("Stage 3 input not found.");
      ...
      for (let i = 0; i < total; i++) {
        ...
        const volP = 50 + (Math.random() * 45);
        const adl = 30 + (Math.random() * 70);
        const force = 45 + (Math.random() * 50);
        const sr = 70 + (Math.random() * 30);
        ...
        results.push({
          ...
          techMetrics: { trend, momentum, volumePattern: volP, adl, forceIndex: force, srLevels: sr },
          sector: item.sector
        });
      ...
      addLog(`Pruned: ${pruned.length} assets pass. Committing to Stage 4 Vault...`, "ok");
      ...
      <div className="xl:col-span-3 space-y-6">
        <div className="glass-panel p-8 md:p-10 rounded-[40px] border-t-2 border-t-orange-500 shadow-2xl bg-slate-900/40 relative overflow-hidden">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
            ...
            <button 
              onClick={executeIntegratedTechAudit}
              disabled={loading}
              className={`px-12 py-5 bg-orange-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-orange-900/20 hover:scale-105 transition-all disabled:opacity-50`}
            >
              {loading ? 'Executing Pattern Scan...' : 'Scan Fundamentals & Commit'}
            </button>
          </div>
          ...
          <div className="bg-black/40 p-8 rounded-3xl border border-white/5 mb-10">
              ...
              <div className="h-2 bg-slate-800 rounded-full overflow-hidden p-0.5">
                <div className="h-full bg-orange-500 transition-all duration-300 rounded-full" style={{ width: `${(progress.current / (progress.total || 1)) * 100}%` }}></div>
              </div>
          </div>
        </div>
      </div>
