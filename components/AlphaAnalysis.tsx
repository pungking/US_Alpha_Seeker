...
const AlphaAnalysis: React.FC<Props> = ({ selectedBrain, setSelectedBrain, onFinalSymbolsDetected, onComplete, autoStart }) => {
  ...
  useEffect(() => {
    if (accessToken) {
      loadStage5Data();
      // 수동 모드일 때도 기존 결과 있으면 복구
      loadLastSynthesis();
    }
  }, [accessToken]);

  // 데이터 로드 성공 후 오토파일럿 트리거
  useEffect(() => {
    if (autoStart && !loading && elite50.length > 0 && !resultsCache[selectedBrain]) {
      addLog("Auto-Pilot Signal: Executing Final Strategic Analysis...", "info");
  ...
  const findFileWithRetry = async (query: string, retries = 5, delay = 3000): Promise<any> => {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        addLog(`Vault Indexing Stage 5... (${i+1}/${retries})`, "warn");
  ...
  const loadStage5Data = async () => {
    ...
    try {
      ...
      if (!file) {
        addLog("Vault Error: Stage 5 input missing. Flow stalled.", "err");
        setLoading(false);
        return;
      }
      ...
      if (content.ict_universe) {
        ...
        addLog(`Vault Synchronized: ${content.ict_universe.length} assets retrieved.`, "ok");
      }
    } catch (e: any) {
  ...
  const loadLastSynthesis = async () => {
    ...
    if (res.files?.length > 0) {
      addLog(`Restoring last synthesis from cloud...`, "info");
      const content = await fetch(`https://www.googleapis.com/drive/v3/files/${res.files[0].id}?alt=media`, {
        headers: { 'Authorization': `Bearer ${accessToken}` }
      }).then(r => r.json());
      if (content.alpha_universe) {
         const brain = res.files[0].name.includes('Gemini') ? ApiProvider.GEMINI : ApiProvider.PERPLEXITY;
  ...
  const executeAlphaFinalization = async () => {
    ...
    // 오토파일럿 시 Gemini 우선 시도
    let targetBrain = autoStart ? ApiProvider.GEMINI : selectedBrain;
    if (autoStart) setSelectedBrain(ApiProvider.GEMINI);

    addLog(`Phase 2: Initiating Neural Synthesis via ${targetBrain === ApiProvider.GEMINI ? 'Gemini 3 Flash' : 'Sonar Pro'}...`, "info");
    ...
    try {
      ...
      // 제미나이 오류 시 퍼플리시티 폴백 (자동화 시에만)
      if (synthesisResponse.error && (autoStart || targetBrain === ApiProvider.GEMINI)) {
        addLog(`Gemini Node Refusal: ${synthesisResponse.error}. Attempting Sonar Fallback...`, "warn");
      ...
      if (synthesisResponse.error) {
        addLog(`Neural Failure: ${synthesisResponse.error}`, "err");
        setLoading(false);
        return;
      }
      ...
      if (accessToken) {
        ...
        const brainTag = targetBrain === ApiProvider.GEMINI ? "Gemini" : "Sonar";
        const fileName = `STAGE6_ALPHA_FINAL_${brainTag}_${new Date().toISOString().split('T')[0]}.json`;
        await uploadFile(accessToken, folderId, fileName, { manifest: { brain: brainTag, count: mergedFinal.length }, alpha_universe: mergedFinal });
        addLog(`Final Vault Committed: ${fileName}`, "ok");
      }

      setProgress(100);
      addLog(`Alpha Protocol Success. Cycle Complete.`, "ok");
      if (onComplete) onComplete();
    } catch (error: any) {
      addLog(`Fatal Node Failure: ${error.message.substring(0, 80)}`, "err");
    } finally {
      setLoading(false);
    }
  };
