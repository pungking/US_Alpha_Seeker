          ...
          <div key={selectedStock.symbol} className="glass-panel p-8 rounded-[50px] bg-slate-950 border-t-2 border-t-rose-600 animate-in fade-in slide-in-from-bottom-8 duration-700 shadow-3xl">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                <div className="lg:col-span-3 space-y-8">
                   ...
                   <div className="p-8 bg-white/5 rounded-[40px] border border-white/10 shadow-inner">
                      ...
                      <div className="prose-report min-h-[200px]">
                        <ReactMarkdown remarkPlugins={[remarkGfm]} components={MarkdownComponents}>{selectedStock.investmentOutlook || "_Analyzing strategic datasets for this asset..._"}</ReactMarkdown>
                      </div>
                   </div>
                </div>
                <div className="lg:col-span-2 space-y-6">
                   ...
                   <button onClick={() => handleRunBacktest(selectedStock)} disabled={backtestLoading} className="w-full py-5 bg-emerald-600/10 border border-emerald-500/20 text-emerald-400 rounded-3xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all shadow-xl active:scale-95">
                     ...
                     <div className="p-6 bg-black/80 rounded-[40px] border border-white/10 shadow-2xl space-y-4 animate-in fade-in slide-in-from-right-4">
                        <div className="grid grid-cols-2 gap-3">
                           {Object.entries(currentBacktest.metrics).map(([k, v]) => (
                             <div key={k} className="p-3 bg-white/5 rounded-2xl border border-white/5 text-center shadow-inner"><p className="text-[7px] text-slate-500 font-black uppercase tracking-tighter mb-1">{k.replace(/([A-Z])/g, ' $1')}</p><p className="text-sm font-black text-white italic">{cleanMarkdown(v)}</p></div>
                           ))}
                        </div>
                        <div className="prose-report text-xs opacity-80 leading-relaxed"><ReactMarkdown remarkPlugins={[remarkGfm]}>{currentBacktest.historicalContext}</ReactMarkdown></div>
                     </div>
                   )}
                </div>
             </div>
