...
const AlphaAnalysis: React.FC<Props> = ({ selectedBrain, setSelectedBrain, onFinalSymbolsDetected }) => {
  ...
  const [logs, setLogs] = useState<string[]>(['> AI_Alpha_Node v8.2.7: Multi-Brain Synthesis Protocol Stable.']);
  ...
  const loadStage5Data = async () => {
    ...
    try {
      ...
      if (listRes.files?.length) {
        ...
        addLog(`Vault Synchronized: ${content.ict_universe?.length} elite candidates ready.`, "ok");
      }
    } catch (e: any) { addLog(`Stage 5 Sync Error: ${e.message}`, "err"); }
  };

  const executeAlphaFinalization = async () => {
    if (elite50.length === 0 || loading) return;
    setLoading(true);
    const brainName = selectedBrain === ApiProvider.GEMINI ? "Google Gemini" : "Perplexity";
    addLog(`Initiating Phase 6 Alpha Synthesis with ${brainName}...`, "info");
    ...
    try {
      ...
      const mergedFinal = (aiResults || []).map(aiData => {
        ...
        return { 
          ...item, 
          ...aiData, 
          entryPrice: aiData.supportLevel || item.price, 
          targetPrice: aiData.resistanceLevel || (aiData.supportLevel || item.price) * 1.3, 
          stopLoss: (aiData.supportLevel || item.price) * 0.9 
        };
      ...
      addLog(`Synthesis Successful: ${mergedFinal.length} market leaders identified.`, "ok");
    } catch (e: any) { 
      addLog(`Synthesis Failed: ${e.message}`, "err"); 
      if (e.message.includes("429")) addLog("Recommendation: Wait 1 minute or switch AI Brain.", "warn");
    }
  ...
  const executeBacktest = async (stock: AlphaCandidate) => {
    ...
    addLog(`Quant Simulation: Performing historical analysis for ${stock.symbol}...`, "info");
    try {
      const { data, error } = await runAiBacktest(stock, selectedBrain);
      if (error) throw new Error(error);
      if (data) {
        setBacktestData(prev => ({ ...prev, [stock.symbol]: data }));
        addLog(`Simulation Confirmed: ${stock.symbol} Win Rate: ${data.metrics.winRate}`, "ok");
      }
    } catch (e: any) { addLog(`Backtest Node Error: ${e.message}`, "err"); }
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
            <div className="flex items-center space-x-6">
              ...
              <div>
                ...
                <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mt-1 italic">Robust Neural Synthesis & Simulation</p>
              </div>
            </div>
            <div className="flex bg-black/40 p-1.5 rounded-2xl border border-white/5">
              {[ApiProvider.GEMINI, ApiProvider.PERPLEXITY].map((p) => (
                <button key={p} onClick={() => setSelectedBrain(p)} className={`px-6 py-3 rounded-xl text-[9px] font-black uppercase transition-all ${selectedBrain === p ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>
                  {p === ApiProvider.GEMINI ? 'Gemini 3 Flash' : 'Sonar Pro'}
                </button>
              ))}
            </div>
            <button onClick={executeAlphaFinalization} disabled={loading || elite50.length === 0} className={`px-10 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl transition-all ${loading ? 'bg-slate-800 text-slate-500 cursor-not-allowed' : 'bg-rose-600 text-white hover:scale-105 active:scale-95'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               ...
               <div key={item.symbol} onClick={() => setSelectedStock(item)} className={`glass-panel p-6 rounded-[32px] border-l-4 cursor-pointer transition-all ${selectedStock?.symbol === item.symbol ? 'border-l-rose-500 bg-rose-500/10 scale-[1.02]' : 'border-l-white/10 hover:bg-white/5'}`}>
                  <div className="flex justify-between items-start mb-2">
                     <div>
                        ...
                        <h4 className="text-3xl font-black text-white italic uppercase tracking-tighter leading-tight">{item.symbol}</h4>
                     </div>
                     <p className="text-xl font-black text-rose-500 italic">{item.convictionScore?.toFixed(0)}%</p>
                  </div>
                  <div className="flex flex-wrap gap-2 mt-4">
                     <span className="text-[7px] px-2 py-0.5 rounded-full font-black border border-white/10 text-slate-400 uppercase tracking-widest truncate max-w-[120px]">{item.chartPattern || 'Analyzing Pattern...'}</span>
             ...
             {!loading && currentResults.length === 0 && (
                <div className="col-span-full py-16 text-center opacity-20">
                   <p className="text-[10px] font-black uppercase tracking-[0.6em] animate-pulse">Awaiting Signal Verification...</p>
                </div>
             )}
          </div>
        </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2 space-y-8">
                   <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                      ...
                      <button onClick={() => executeBacktest(selectedStock)} disabled={backtestLoading} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${backtestLoading ? 'bg-slate-800 border-white/10 text-slate-500 cursor-not-allowed' : 'bg-emerald-600/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-600 hover:text-white shadow-lg shadow-emerald-900/20'}`}>
                        {backtestLoading ? 'Analyzing History...' : 'Run AI Backtest'}
                      </button>
                   </div>
                   ...
                   {currentBacktest && (
                     <div className="grid grid-cols-1 gap-6 animate-in zoom-in-95 duration-500">
                        ...
                        <div className="h-64 w-full bg-black/40 rounded-3xl border border-white/5 p-6 shadow-inner">
                           <p className="text-[10px] font-black text-slate-500 uppercase mb-4 tracking-widest italic opacity-50">Historical Equity Curve Projection</p>
                        ...
                        <div className="p-6 bg-emerald-500/5 rounded-2xl border border-emerald-500/20">
                           <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-3 italic">Strategy Validation Summary</p>
                   ...
                   <div className="grid grid-cols-3 gap-4">
                      <div className="p-6 bg-emerald-500/5 rounded-2xl border border-emerald-500/10">
                         ...
                         <p className="text-xl font-mono font-black text-white">${selectedStock.supportLevel?.toFixed(2) || selectedStock.price?.toFixed(2)}</p>
                      </div>
                      <div className="p-6 bg-blue-500/5 rounded-2xl border border-blue-500/10">
                         <p className="text-[8px] font-black text-blue-500 uppercase mb-1 tracking-widest">Resistance (Target)</p>
                         <p className="text-xl font-mono font-black text-white">${selectedStock.resistanceLevel?.toFixed(2) || (selectedStock.price * 1.3).toFixed(2)}</p>
                      </div>
                      <div className="p-6 bg-rose-500/5 rounded-2xl border border-rose-500/10">
                         ...
                         <p className="text-xl font-mono font-black text-white">${selectedStock.stopLoss?.toFixed(2) || (selectedStock.price * 0.9).toFixed(2)}</p>
                      </div>
                   </div>

                   <div className="bg-black/60 rounded-[32px] border border-white/5 aspect-video overflow-hidden shadow-2xl relative group">
                      <iframe title="Live Chart" src={`https://s.tradingview.com/widgetembed/?symbol=${selectedStock.symbol}&interval=D&theme=dark&style=1&timezone=Etc%2FUTC`} className="w-full h-full border-none" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent pointer-events-none group-hover:opacity-0 transition-opacity"></div>
                   </div>

                   <div className="p-10 bg-white/5 rounded-[32px] border border-white/5 border-l-4 border-l-rose-500/30">
                      <h4 className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-6 italic">Strategic Insight Matrix</h4>
                ...
                <div className="space-y-8 pt-4">
                   <div className="p-10 bg-rose-500/10 rounded-[40px] border border-rose-500/20 shadow-xl relative overflow-hidden">
                      <div className="absolute top-0 right-0 p-4 opacity-10">
                         <svg className="w-16 h-16 text-rose-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L4.5 20.29l.71.71L12 18l6.79 3 .71-.71L12 2z"/></svg>
                      </div>
                      <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-6">Neural Conviction Index</p>
                      <div className="flex items-center space-x-6 mb-6">
                         <div className="h-3 flex-1 bg-slate-800 rounded-full overflow-hidden shadow-inner">
                            <div className="h-full bg-gradient-to-r from-rose-600 to-rose-400 shadow-[0_0_12px_rgba(244,63,94,0.4)]" style={{ width: `${selectedStock.convictionScore || 50}%` }}></div>
                         </div>
                         <span className="text-xl font-black text-white italic">{selectedStock.convictionScore?.toFixed(0)}%</span>
                      </div>
                      <p className="text-[10px] text-slate-400 italic leading-relaxed uppercase tracking-tighter">{selectedStock.aiSentiment}</p>
                   </div>
                   
                   <div className="p-8 bg-white/5 rounded-[32px] border border-white/5">
                      <p className="text-[9px] font-black text-slate-500 uppercase mb-4 tracking-widest italic">Core Selection Reasoning</p>
                      <ul className="space-y-3">
                        {selectedStock.selectionReasons?.map((r, i) => (
                          <li key={i} className="flex items-start space-x-3 group">
                             <div className="w-1.5 h-1.5 rounded-full bg-rose-500 mt-1.5 shrink-0 group-hover:scale-150 transition-transform"></div>
                             <p className="text-[11px] font-bold text-slate-400 leading-tight uppercase group-hover:text-white transition-colors">{r}</p>
                          </li>
                        ))}
                      </ul>
                   </div>

                   <div className="p-8 bg-black/40 rounded-[32px] border border-white/5 border-l-4 border-l-rose-500">
                      <p className="text-[9px] font-black text-slate-600 uppercase mb-4 tracking-widest">Neural Analysis Logic</p>
                      <p className="text-[11px] text-slate-400 leading-relaxed italic uppercase font-mono tracking-tighter">{selectedStock.analysisLogic}</p>
                   </div>
                </div>
             </div>
          </div>
      ...
      <div className="xl:col-span-1">
        <div className="glass-panel h-[720px] rounded-[40px] bg-slate-950 border-l-4 border-l-rose-600 flex flex-col p-6 shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between mb-8 px-2">
            ...
            <div className={`w-2 h-2 rounded-full ${loading || backtestLoading ? 'bg-rose-500 animate-pulse shadow-[0_0_10px_rgba(244,63,94,0.8)]' : 'bg-slate-700'}`}></div>
          </div>
