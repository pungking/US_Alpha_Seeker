...
interface AlphaCandidate {
  ...
  chosenPlanType?: 'PULLBACK' | 'BREAKOUT' | 'ADAPTIVE_CURRENT' | 'ADAPTIVE_RECALC_STOP' | 'NO_TRADE';
  entryTactic?:
    | 'PULLBACK_LIMIT'
  ...
  currentEntryStructureSupportReference?: string | null;
  currentEntryStructureSupportGapAtr?: number | null;
  currentEntryStructureNearestSupportLow?: number | null;
  currentEntryStructureNearestSupportGapAtr?: number | null;
  currentEntryStructureStopAlignedSupportLow?: number | null;
  currentEntryStructureStopAlignedSupportGapAtr?: number | null;
  currentEntryStructurePriceDriftPct?: number | null;
  currentEntryStructureStopSupportRelation?: string | null;
  currentEntryStructureSupportStopGapPct?: number | null;
  currentEntryStructureStopAboveSupportPct?: number | null;
  currentEntryStructureStopBelowSupportPct?: number | null;
  ...
  breakoutRetestProofMaxBarsSinceRetest?: number | null;
  ...
  breakoutRetestProofUndercutReclaimFound?: boolean | null;
  breakoutRetestProofUndercutExcessPct?: number | null;
  breakoutRetestProofMaxUndercutExcessPct?: number | null;
  ...
  breakoutRetestProofContinuationExtensionOk?: boolean | null;
  ...
  targetRecalibrationRequiredTargetByExecutionFloorPrice?: number | null;
  targetRecalibrationRequiredTargetByExpectedReturnPrice?: number | null;
  ...
  targetRecalibrationExecutionFloorGapPct?: number | null;
  targetRecalibrationExpectedReturnGapPct?: number | null;
  targetRecalibrationExecutionFloorShortfallPct?: number | null;
  targetRecalibrationExpectedReturnShortfallPct?: number | null;
  targetRecalibrationExecutionFloorViable?: boolean | null;
  targetRecalibrationRequiredTargetSource?: string | null;
  targetRecalibrationRequiredTargetDominantReason?: string | null;
  ...
  riskGeometryProofVerdict?: string | null;
  riskGeometryRecalculatedStopPrice?: number | null;
  riskGeometryRecalculatedStopDistancePct?: number | null;
  riskGeometryRrAtRecalculatedStop?: number | null;
  riskGeometryRrAtRequiredTargetAndRecalculatedStop?: number | null;
  ...
  riskGeometryRequiredTargetDominantReason?: string | null;
  ...
  riskGeometryStopDistancePolicyVerdict?: string | null;
  riskGeometryTargetShortfallPolicyVerdict?: string | null;
  riskGeometryRecalculatedStopRrOk?: boolean | null;
  riskGeometryTargetBufferOk?: boolean | null;
  riskGeometryTargetBufferAtRequiredTargetPct?: number | null;
  riskGeometryTargetRecalibrationProofReady?: boolean | null;
  ...
  executionActionablePolicy?: string | null;
  executionActionableWaiver?: boolean;
  executionActionableWaiverReason?: string | null;
...
type CurrentEntryStructurePayload = {
  ...
  supportGapAtr: number | null;
  nearestSupportLow: number | null;
  nearestSupportGapAtr: number | null;
  stopAlignedSupportLow: number | null;
  stopAlignedSupportGapAtr: number | null;
  priceDriftPct: number | null;
  stopSupportRelation: string | null;
  supportStopGapPct: number | null;
  stopAboveSupportPct: number | null;
  stopBelowSupportPct: number | null;
};
...
type BreakoutRetestProofPayload = {
  ...
  maxBarsSinceRetest: number | null;
  maxCurrentExtensionFromRetestPct: number | null;
  retestTouchFound: boolean;
  retestFresh: boolean;
  retestCloseReclaimed: boolean;
  undercutReclaimFound: boolean;
  undercutExcessPct: number | null;
  maxUndercutExcessPct: number | null;
  currentExtensionOk: boolean;
  latestCloseAboveRetest: boolean;
  continuationConfirmed: boolean;
  continuationExtensionOk: boolean;
  maxContinuationExtensionPct: number | null;
  continuationMinRr: number | null;
...
type Stage6TargetPolicyPayload = {
  ...
  requiredTargetByExecutionFloorPrice: number | null;
  ...
  executionFloorGapPct: number | null;
  expectedReturnGapPct: number | null;
  executionFloorShortfallPct: number | null;
  expectedReturnShortfallPct: number | null;
  executionFloorViable: boolean;
  requiredTargetSource: string | null;
  requiredTargetDominantReason: string | null;
...
type Stage6RiskGeometryPolicyPayload = {
  ...
  rrAtRequiredTargetAndRecalculatedStop: number | null;
  requiredTargetPrice: number | null;
  ...
  stopDistancePolicyVerdict: string;
  targetShortfallPolicyVerdict: string;
  recalculatedStopRrOk: boolean;
  targetBufferOk: boolean;
  targetBufferAtRequiredTargetPct: number | null;
  targetRecalibrationProofReady: boolean;
  repairLane: string;
...
const BREAKOUT_RETEST_PROOF_POLICY = {
  ...
  maxReclaimUndercutExcessPct: 1.0,
  maxCurrentExtensionFromRetestPct: 8.0,
  maxContinuationExtensionPct: 25.0,
  continuationMinRrMultiplier: 1.5,
  continuationMinTargetBufferMultiplier: 3.0
};

const TARGET_RECALIBRATION_POLICY = {
  maxRequiredTargetGapPct: 20.0,
  maxExecutionFloorGapPct: 8.0
};
...
const supportAlignedWithStop = (
  ...
  if (!(price > 0 && requiredStop > 0 && atr > 0)) return null;
  const recent = bars.slice(-lookback);
  const swings: Stage6StructureBar[] = [];
  for (let i = 1; i < recent.length - 1; i += 1) {
    if (recent[i].low <= recent[i - 1].low && recent[i].low <= recent[i + 1].low && recent[i].low < price) {
  ...
  const candidates = (swings.length ? swings : recent.filter((bar) => bar.low < price))
    .map((bar) => ({ bar, gapAtr: Math.abs((bar.low - requiredStop) / atr) }))
    .filter(({ gapAtr }) => gapAtr <= maxGapAtr)
    .sort((a, b) => a.gapAtr - b.gapAtr || b.bar.low - a.bar.low);
  return candidates[0]?.bar ?? null;
};
...
const deriveCurrentEntryStructure = (input: {
  ...
  const basePayload = {
    ...
    supportReference: null as string | null,
    supportGapAtr: null as number | null,
    nearestSupportLow: null as number | null,
    nearestSupportGapAtr: null as number | null,
    stopAlignedSupportLow: null as number | null,
    stopAlignedSupportGapAtr: null as number | null,
    stopSupportRelation: null as string | null,
    supportStopGapPct: null as number | null,
    stopAboveSupportPct: null as number | null,
    stopBelowSupportPct: null as number | null,
    priceDriftPct:
  ...
  const nearestSupport = nearestSupportBelowPrice(bars, price, policy.swingLookback);
  const stopAlignedSupport = supportAlignedWithStop(
    bars,
    price,
    requiredStop,
    atr,
    policy.swingLookback,
    policy.supportBufferAtr
  );
  const support = stopAlignedSupport ?? nearestSupport;
  const supportReference = stopAlignedSupport ? 'stop_aligned_swing_support' : nearestSupport ? 'nearest_swing_support_below_price' : null;
  ...
  const nearestSupportGapAtr = nearestSupport?.low ? (nearestSupport.low - requiredStop) / atr : null;
  const stopAlignedSupportGapAtr = stopAlignedSupport?.low ? Math.abs((stopAlignedSupport.low - requiredStop) / atr) : null;
  const supportStopGapPct =
    support?.low && price > 0 ? ((support.low - requiredStop) / price) * 100 : null;
  const stopAboveSupportPct =
    supportStopGapPct != null && supportStopGapPct < 0 ? Math.abs(supportStopGapPct) : null;
  const stopBelowSupportPct =
    supportStopGapPct != null && supportStopGapPct > 0 ? supportStopGapPct : null;
  const stopSupportRelation = !support
    ? 'support_missing'
    : supportStopGapPct == null
      ? 'support_stop_gap_unavailable'
      : supportStopGapPct < 0
        ? 'stop_above_support'
        : supportGapAtr != null && supportGapAtr > policy.supportBufferAtr
          ? 'stop_below_support_beyond_buffer'
          : 'stop_below_support_within_buffer';
  ...
  return {
    ...
    supportReference,
    supportGapAtr: roundOrNull(supportGapAtr, 2),
    nearestSupportLow: roundOrNull(nearestSupport?.low ?? null, 4),
    nearestSupportGapAtr: roundOrNull(nearestSupportGapAtr, 2),
    stopAlignedSupportLow: roundOrNull(stopAlignedSupport?.low ?? null, 4),
    stopAlignedSupportGapAtr: roundOrNull(stopAlignedSupportGapAtr, 2),
    stopSupportRelation,
    supportStopGapPct: roundOrNull(supportStopGapPct, 2),
    stopAboveSupportPct: roundOrNull(stopAboveSupportPct, 2),
    stopBelowSupportPct: roundOrNull(stopBelowSupportPct, 2),
...
const deriveBreakoutRetestProof = (input: {
  ...
  const minTargetBufferPct =
    input.minTargetBufferPct != null && Number.isFinite(input.minTargetBufferPct) && input.minTargetBufferPct > 0
      ...
      : 2;
  const continuationMinRr = minRr * BREAKOUT_RETEST_PROOF_POLICY.continuationMinRrMultiplier;
  const continuationMinTargetBufferPct = minTargetBufferPct * BREAKOUT_RETEST_PROOF_POLICY.continuationMinTargetBufferMultiplier;
  const bars = normalizeStage6StructureBars(input.priceHistory);
  const latest = bars[bars.length - 1] || null;
  const price = input.price;
  ...
  const basePayload = {
    ...
    currentExtensionFromRetestPct:
      price != null && entry != null && entry > 0
        ...
        : null,
    tolerancePct: roundOrNull(policy.retestTolerancePct, 2),
    maxBarsSinceRetest: policy.maxBarsSinceRetest,
    ...
    undercutReclaimFound: false,
    undercutExcessPct: null as number | null,
    maxUndercutExcessPct: roundOrNull(policy.maxReclaimUndercutExcessPct, 2),
    currentExtensionOk: false,
    latestCloseAboveRetest: false,
    continuationConfirmed: false,
    continuationExtensionOk: false,
  ...
  const retestUndercutReclaimCandidates = retestUndercutCandidates
    .map((candidate) => ({
      ...candidate,
      undercutExcessPct: Math.max(0, ((entry - candidate.bar.low) / entry) * 100 - policy.retestTolerancePct)
    }))
    .filter(({ bar, undercutExcessPct }) => bar.close >= entry && undercutExcessPct <= policy.maxReclaimUndercutExcessPct);
  ...
  const latestRetestUndercutReclaim = retestUndercutReclaimCandidates[retestUndercutReclaimCandidates.length - 1] || null;
  const effectiveRetest = latestRetest || latestRetestUndercutReclaim || null;
  const barsSinceRetest = effectiveRetest ? recent.length - 1 - effectiveRetest.idx : null;
  const currentExtensionFromRetestPct = ((price - entry) / entry) * 100;
  const undercutReclaimFound = Boolean(!latestRetest && latestRetestUndercutReclaim);
  const retestTouchFound = Boolean(effectiveRetest);
  const retestFresh = barsSinceRetest != null && barsSinceRetest <= policy.maxBarsSinceRetest;
  const retestCloseReclaimed = Boolean(effectiveRetest && effectiveRetest.bar.close >= entry);
  const retestLowGapPct = effectiveRetest ? ((entry - effectiveRetest.bar.low) / entry) * 100 : null;
  const retestCloseGapPct = effectiveRetest ? ((effectiveRetest.bar.close - entry) / entry) * 100 : null;
  const latestCloseGapPct = latest ? ((latest.close - entry) / entry) * 100 : null;
  const retestUndercutExcessPct =
    latestRetestUndercut && !effectiveRetest
      ? Math.max(0, ((entry - latestRetestUndercut.bar.low) / entry) * 100 - policy.retestTolerancePct)
      : undercutReclaimFound && latestRetestUndercutReclaim
        ? latestRetestUndercutReclaim.undercutExcessPct
      : 0;
  ...
  const continuationCurrentRrOk =
    input.rrAtCurrentPrice != null &&
    Number.isFinite(input.rrAtCurrentPrice) &&
    input.rrAtCurrentPrice >= continuationMinRr;
  const continuationTargetBufferOk =
    input.targetBufferFromCurrentPct != null &&
    Number.isFinite(input.targetBufferFromCurrentPct) &&
    input.targetBufferFromCurrentPct >= continuationMinTargetBufferPct;
  const continuationConfirmed = Boolean(
    !retestTouchFound &&
    latestCloseAboveRetest &&
    continuationExtensionOk &&
    continuationCurrentRrOk &&
    continuationTargetBufferOk
  );
  const reasons: string[] = [];
  if (!retestTouchFound) reasons.push('retest_touch_missing');
  if (undercutReclaimFound) reasons.push('retest_undercut_reclaim_within_policy');
  ...
  const fallbackBreakoutBasis = !retestTouchFound
    ? latestRetestUndercut
      ? 'breakout_retest_undercut_reclaim_gap'
  ...
  const verdict = confirmed
    ? retestConfirmed
      ...
      : 'BREAKOUT_CONTINUATION_CONFIRMED_CURRENT_ENTRY_CANDIDATE'
    : reviewReady
  ...
  return {
    ...
    reasons: confirmed
      ? retestConfirmed
        ? undercutReclaimFound
          ? ['undercut_reclaim_retest_proof_confirmed']
          : ['retest_proof_confirmed']
        : ['continuation_breakout_confirmed_without_retest_touch']
      : reasons,
    ...basePayload,
    retestDate: effectiveRetest?.bar?.date ?? null,
    retestLow: roundOrNull(effectiveRetest?.bar?.low ?? null, 4),
    retestClose: roundOrNull(effectiveRetest?.bar?.close ?? null, 4),
    ...
    undercutReclaimFound,
    undercutExcessPct: roundOrNull(retestUndercutExcessPct, 2),
    maxUndercutExcessPct: roundOrNull(policy.maxReclaimUndercutExcessPct, 2),
    currentExtensionOk,
    latestCloseAboveRetest,
...
const deriveTargetRecalibrationPolicy = (input: {
  ...
  const currentRrWeak =
    ...
    !Number.isFinite(input.rrAtCurrentPrice) ||
    input.rrAtCurrentPrice < minRr;
  const actualStopDistanceAtCurrent =
    input.price != null &&
    input.stop != null &&
    Number.isFinite(input.price) &&
    ...
    input.price > 0 &&
    input.stop > 0 &&
    input.stop < input.price
      ? input.price - input.stop
      : null;
  const inferredStopDistanceAtCurrent =
    input.rrAtCurrentPrice != null &&
    Number.isFinite(input.rrAtCurrentPrice) &&
    input.rrAtCurrentPrice > 0 &&
    input.target != null &&
    input.price != null &&
    Number.isFinite(input.target) &&
    Number.isFinite(input.price) &&
    input.target > input.price
      ? (input.target - input.price) / input.rrAtCurrentPrice
      : null;
  const stopDistanceAtCurrent = actualStopDistanceAtCurrent ?? inferredStopDistanceAtCurrent;
  const riskBasisStopDistancePct =
    input.price != null && input.price > 0 && stopDistanceAtCurrent != null
      ? (stopDistanceAtCurrent / input.price) * 100
      : null;
  const requiredTargetByBuffer =
    input.price != null && Number.isFinite(input.price) && input.price > 0
      ? input.price * (1 + minTargetBufferPct / 100)
      : null;
  const requiredTargetByRr =
    input.price != null &&
    Number.isFinite(input.price) &&
    input.price > 0 &&
    stopDistanceAtCurrent != null &&
    Number.isFinite(stopDistanceAtCurrent) &&
    stopDistanceAtCurrent > 0
      ? input.price + stopDistanceAtCurrent * minRr
      : null;
  const requiredTargetByExecutionFloor =
    [requiredTargetByBuffer, requiredTargetByRr]
  ...
  const requiredTargetPrice =
    [requiredTargetByBuffer, requiredTargetByRr, requiredTargetByExpectedReturn]
      .filter((value): value is number => value != null && Number.isFinite(value) && value > 0)
      .reduce<number | null>((maxValue, value) => (maxValue == null ? value : Math.max(maxValue, value)), null);
  const requiredTargetSource =
    requiredTargetByExpectedReturn != null && requiredTargetPrice === requiredTargetByExpectedReturn
      ? actualStopDistanceAtCurrent != null
        ? 'expected_return_and_actual_stop_risk'
        : inferredStopDistanceAtCurrent != null
          ? 'expected_return_and_inferred_rr_risk'
          : 'expected_return'
      : actualStopDistanceAtCurrent != null
        ? 'actual_stop_risk'
        : inferredStopDistanceAtCurrent != null
          ? 'inferred_rr_risk'
          : 'target_buffer_only';
  const requiredTargetDominantReason = requiredTargetSource?.includes('expected_return')
    ? 'expected_return_required_target_dominates'
    : requiredTargetSource?.includes('stop_risk') || requiredTargetSource?.includes('rr_risk')
      ? 'risk_stop_required_target_dominates'
      : 'target_buffer_required_target_dominates';
  ...
  const currentTargetGapPct =
    ...
    requiredTargetPrice > 0
      ...
      : null;
  const executionFloorGapPct =
    requiredTargetByExecutionFloor != null &&
    input.target != null &&
    Number.isFinite(input.target) &&
    requiredTargetByExecutionFloor > 0
      ? ((input.target - requiredTargetByExecutionFloor) / requiredTargetByExecutionFloor) * 100
      : null;
  const expectedReturnGapPct =
    requiredTargetByExpectedReturn != null &&
    input.target != null &&
    Number.isFinite(input.target) &&
    requiredTargetByExpectedReturn > 0
      ? ((input.target - requiredTargetByExpectedReturn) / requiredTargetByExpectedReturn) * 100
      : null;
  const targetRecalibrationGapPolicyPct = TARGET_RECALIBRATION_POLICY.maxRequiredTargetGapPct;
  const executionFloorGapPolicyPct = TARGET_RECALIBRATION_POLICY.maxExecutionFloorGapPct;
  const currentTargetShortfallPct =
    currentTargetGapPct != null && Number.isFinite(currentTargetGapPct) && currentTargetGapPct < 0
      ...
      : 0;
  const executionFloorShortfallPct =
    executionFloorGapPct != null && Number.isFinite(executionFloorGapPct) && executionFloorGapPct < 0
      ? Math.abs(executionFloorGapPct)
      : 0;
  const expectedReturnShortfallPct =
    expectedReturnGapPct != null && Number.isFinite(expectedReturnGapPct) && expectedReturnGapPct < 0
      ? Math.abs(expectedReturnGapPct)
      : 0;
  ...
  const executionFloorViable =
    ...
    executionFloorShortfallPct <= executionFloorGapPolicyPct;
  ...
  const viabilityVerdict = targetRecalibrationCandidate
    ...
    : targetNoTradeConfirmed
      ? targetAlreadyReached
        ...
        : executionFloorViable
          ? 'TARGET_EXECUTION_FLOOR_VIABLE_EXPECTED_RETURN_GAP_TOO_WIDE'
          : currentTargetShortfallPct > targetRecalibrationGapPolicyPct
  ...
  const targetEvidence = {
    ...
    requiredTargetByExecutionFloorPrice: roundOrNull(requiredTargetByExecutionFloor, 4),
    ...
    executionFloorGapPct: roundOrNull(executionFloorGapPct, 2),
    expectedReturnGapPct: roundOrNull(expectedReturnGapPct, 2),
    executionFloorShortfallPct: roundOrNull(executionFloorShortfallPct, 2),
    expectedReturnShortfallPct: roundOrNull(expectedReturnShortfallPct, 2),
    executionFloorViable,
    requiredTargetSource,
    requiredTargetDominantReason,
  ...
  const reasons = [
    ...
    ...(requiredTargetPrice != null && input.target != null && input.target < requiredTargetPrice
      ? ['current_target_below_required_target']
      : [])
  ];
  if (targetAlreadyReached) {
    return {
...
const deriveRiskGeometryPolicy = (input: {
  ...
  if (!applicableReasons.has(reason)) {
    return {
      ...
      proofVerdict: 'RISK_GEOMETRY_PROOF_NOT_APPLICABLE',
      ...
      requiredTargetDominantReason: null,
      ...
      stopDistancePolicyVerdict: 'not_applicable',
  ...
  const rrAtRecalculatedStop =
    ...
    Number.isFinite(input.target) &&
    Number.isFinite(input.currentEntryRequiredStopPrice) &&
    input.price > input.currentEntryRequiredStopPrice &&
    input.target > input.price
      ? (input.target - input.price) / (input.price - input.currentEntryRequiredStopPrice)
      : null;
  const requiredTargetByRecalculatedStop =
    input.price != null &&
    input.currentEntryRequiredStopPrice != null &&
    Number.isFinite(input.price) &&
    Number.isFinite(input.currentEntryRequiredStopPrice) &&
    input.price > input.currentEntryRequiredStopPrice &&
    input.currentEntryRequiredStopPrice > 0
      ? input.price + (input.price - input.currentEntryRequiredStopPrice) * input.minRr
      : null;
  const requiredTargetByBuffer =
    input.price != null && Number.isFinite(input.price) && input.price > 0
  ...
  const requiredTargetDominantReason = requiredTargetSource === 'expected_return'
    ? 'expected_return_required_target_dominates'
    : requiredTargetSource === 'stop_min_rr'
      ? 'min_rr_required_target_dominates'
      : requiredTargetSource === 'target_buffer'
        ? 'target_buffer_required_target_dominates'
        : 'required_target_unavailable';
  const rrAtRequiredTargetAndRecalculatedStop =
    requiredTargetPrice != null &&
    input.price != null &&
    input.currentEntryRequiredStopPrice != null &&
    Number.isFinite(requiredTargetPrice) &&
    Number.isFinite(input.price) &&
    Number.isFinite(input.currentEntryRequiredStopPrice) &&
    input.price > input.currentEntryRequiredStopPrice &&
    requiredTargetPrice > input.price
      ? (requiredTargetPrice - input.price) / (input.price - input.currentEntryRequiredStopPrice)
      : null;
  const requiredTargetBufferPct =
    requiredTargetPrice != null && input.price != null && Number.isFinite(input.price) && input.price > 0
      ? ((requiredTargetPrice - input.price) / input.price) * 100
  ...
  const recalculatedStopRrOk =
    ...
    rrAtRecalculatedStop >= input.minRr;
  const requiredTargetRrOk =
    rrAtRequiredTargetAndRecalculatedStop != null &&
    Number.isFinite(rrAtRequiredTargetAndRecalculatedStop) &&
    rrAtRequiredTargetAndRecalculatedStop >= input.minRr;
  const requiredTargetBufferOk =
    requiredTargetBufferPct != null &&
    Number.isFinite(requiredTargetBufferPct) &&
    requiredTargetBufferPct >= input.minTargetBufferPct;
  ...
  const targetRecalibrationProofReady = Boolean(
    targetRecalibrationCandidate &&
    requiredTargetRrOk &&
    requiredTargetBufferOk
  );
  const stopDistancePolicyVerdict = !requiredStopValid
    ? 'REQUIRED_STOP_INVALID'
    : requiredStopDistanceValid
      ? 'REQUIRED_STOP_DISTANCE_OK'
      : input.currentEntryRequiredStopDistancePct != null &&
        Number.isFinite(input.currentEntryRequiredStopDistancePct) &&
        input.currentEntryRequiredStopDistancePct < input.minStopDistancePct
        ? 'REQUIRED_STOP_DISTANCE_BELOW_MIN'
        : input.currentEntryRequiredStopDistancePct != null &&
          Number.isFinite(input.currentEntryRequiredStopDistancePct) &&
          input.currentEntryRequiredStopDistancePct > input.maxStopDistancePct
          ? 'REQUIRED_STOP_DISTANCE_ABOVE_MAX'
          : 'REQUIRED_STOP_DISTANCE_UNAVAILABLE';
  const targetShortfallPolicyVerdict = !targetAboveCurrent
    ? 'TARGET_NOT_ABOVE_CURRENT_NO_TRADE'
    : targetNoTradeConfirmed
      ? 'TARGET_GAP_TOO_WIDE_NO_TRADE'
      : targetRecalibrationProofReady
        ? 'TARGET_RECALIBRATION_PROOF_READY'
        : targetRecalibrationCandidate
          ? 'TARGET_RECALIBRATION_CANDIDATE'
          : targetShortfallPct != null && targetShortfallPct > 0
            ? 'TARGET_SHORTFALL_REVIEW'
            : 'TARGET_GEOMETRY_OK';
  ...
  const repairLane = !targetAboveCurrent
    ...
    : targetNoTradeConfirmed
      ...
      : targetRecalibrationProofReady
        ? 'RECALCULATED_STOP_TARGET_RECALIBRATION_REVIEW'
        : targetRecalibrationCandidate
          ? 'TARGET_RECALIBRATION'
  ...
  const proofReasons = [
    ...
    ...(requiredStopValid ? [] : ['required_stop_invalid']),
    ...(requiredStopDistanceValid ? [] : ['required_stop_distance_out_of_policy']),
    ...
    ...(targetShortfallPct != null && targetShortfallPct > targetRecalibrationGapPolicyPct
      ...
      : []),
    ...(targetRecalibrationProofReady ? ['target_recalibration_required_target_rr_buffer_ok'] : []),
  ...
  const proofVerdict = recalculatedStopProofConfirmed
    ? 'RECALCULATED_STOP_PROOF_CONFIRMED'
    : targetRecalibrationProofReady
      ? 'RECALCULATED_STOP_TARGET_RECALIBRATION_PROOF_READY'
      : recalculatedStopCandidate
  ...
  const proofEvidence = {
    ...
    rrAtRequiredTargetAndRecalculatedStop: roundOrNull(rrAtRequiredTargetAndRecalculatedStop, 2),
    requiredTargetPrice: roundOrNull(requiredTargetPrice, 4),
    ...
    requiredStopValid,
    requiredStopDistanceValid,
    stopDistancePolicyVerdict,
    targetShortfallPolicyVerdict,
    recalculatedStopRrOk,
    targetBufferOk,
    targetBufferAtRequiredTargetPct: roundOrNull(requiredTargetBufferPct, 2),
    targetRecalibrationProofReady,
    repairLane,
  ...
  const reasons = [
    ...
    ...(recalculatedStopProofConfirmed ? ['recalculated_stop_proof_confirmed'] : []),
    ...(!input.currentEntryRecalcFeasible ? ['current_entry_recalc_not_feasible'] : []),
  ...
  if (reason === 'wait_target_near_current' || reason === 'blocked_target_too_close') {
    return {
	  ...
	  if (reason === 'blocked_stop_too_tight' || reason === 'blocked_stop_too_wide') {
	    return {
	      verdict: targetNoTradeConfirmed
	        ...
	        : targetRecalibrationProofReady
	          ? 'STOP_GEOMETRY_TARGET_RECALIBRATION_REVIEW_READY'
	        : recalculatedStopCandidate
	  ...
	  if (reason === 'wait_recalculated_stop_required') {
	    return {
	      verdict: targetNoTradeConfirmed
	        ...
	        : targetRecalibrationProofReady
	          ? 'RECALCULATED_STOP_TARGET_RECALIBRATION_REVIEW_READY'
	        : recalculatedStopCandidate
	  ...
	  return {
	    verdict: targetNoTradeConfirmed
	      ...
	      : targetRecalibrationProofReady
	        ? 'RR_GEOMETRY_TARGET_RECALIBRATION_REVIEW_READY'
    ...
    ...proofEvidence,
    reasons,
    recommendedAction: recalculatedStopProofConfirmed
...
const deriveZeroExecutableTuningPolicy = (input: {
  decisionReason: string | null;
...
const SIGNAL_DEFINITIONS: Record<string, { title: string; desc: string }> = {
    ...
    'REASON_EXECUTABLE_CURRENT_RECALCULATED_STOP': {
        ...
        desc: "현재가 진입을 위해 필요한 손절 재산정값이 허용 범위 안에 있습니다. 명시적 설정이 켜진 경우에만 실행 후보로 인정합니다."
    },
    ...
    'REASON_WAIT_RECALCULATED_STOP_REQUIRED': {
...
const AlphaAnalysis: React.FC<Props> = ({ selectedBrain, setSelectedBrain, onFinalSymbolsDetected, onStockSelected, analyzingSymbols = new Set(), autoStart, onComplete, isVisible = true }) => {
  ...
  const getDecisionLabel = (decision?: string | null) => {
      ...
      if (key === 'executable_current_recalculated_stop') return 'REASON_EXECUTABLE_CURRENT_RECALCULATED_STOP';
...
- **기대 수익/손익비** : ${expectedLine}`;
  ...
  const runStage2 = async (candidates: AlphaCandidate[]) => {
      ...
      const stage6ActionableVerdictKeys = String((import.meta as any)?.env?.VITE_STAGE6_ACTIONABLE_VERDICTS ?? 'BUY,STRONG_BUY,STRONGBUY')
          .split(',')
          .map((value) => toVerdictKey(value))
          .filter(Boolean);
      const STAGE6_ACTIONABLE_VERDICTS = new Set(
          stage6ActionableVerdictKeys.length > 0 ? stage6ActionableVerdictKeys : ['BUY', 'STRONG_BUY', 'STRONGBUY']
      );
      ...
      const isSidecarActionableVerdictForExecution = (verdict: string | null | undefined) => {
          const key = toVerdictKey(verdict);
          if (!key || key === 'NA' || key === 'N/A' || key === 'NONE' || key === 'NULL' || key === 'UNDEFINED' || key === 'TBD') {
              return false;
          }
          if (STAGE6_ACTIONABLE_VERDICTS.has(key)) return true;
          const speculative = key.includes('SPECULATIVE_BUY') || key.includes('SPECULATIVEBUY');
          return speculative && STAGE6_SPECULATIVE_BUY_EXECUTABLE_WAIVER;
      };
      ...
      const deriveExecutionContractFields = (item: AlphaCandidate) => {
          ...
          const currentPriceStopDistancePct =
              livePrice != null && mirroredStop != null && livePrice > 0 && mirroredStop > 0
                  ...
                  : null;
          const currentEntryRequiredStopPrice =
              livePrice != null &&
              mirroredTarget != null &&
              livePrice > 0 &&
              mirroredTarget > livePrice &&
              STAGE6_CURRENT_ENTRY_MIN_RR > 0
                  ? Number((livePrice - ((mirroredTarget - livePrice) / STAGE6_CURRENT_ENTRY_MIN_RR)).toFixed(4))
                  : null;
          const currentEntryRequiredStopDistancePct =
              livePrice != null &&
              currentEntryRequiredStopPrice != null &&
              livePrice > 0 &&
              currentEntryRequiredStopPrice > 0 &&
          ...
          const currentEntryStructureSupportReference = derivedCurrentEntryStructure.supportReference;
          const currentEntryStructureSupportGapAtr = derivedCurrentEntryStructure.supportGapAtr;
          const currentEntryStructureNearestSupportLow = derivedCurrentEntryStructure.nearestSupportLow;
          const currentEntryStructureNearestSupportGapAtr = derivedCurrentEntryStructure.nearestSupportGapAtr;
          const currentEntryStructureStopAlignedSupportLow = derivedCurrentEntryStructure.stopAlignedSupportLow;
          const currentEntryStructureStopAlignedSupportGapAtr = derivedCurrentEntryStructure.stopAlignedSupportGapAtr;
          const currentEntryStructurePriceDriftPct = derivedCurrentEntryStructure.priceDriftPct;
          const currentEntryStructureStopSupportRelation = derivedCurrentEntryStructure.stopSupportRelation;
          const currentEntryStructureSupportStopGapPct = derivedCurrentEntryStructure.supportStopGapPct;
          const currentEntryStructureStopAboveSupportPct = derivedCurrentEntryStructure.stopAboveSupportPct;
          const currentEntryStructureStopBelowSupportPct = derivedCurrentEntryStructure.stopBelowSupportPct;
          ...
          const executionActionableVerdict = isSidecarActionableVerdictForExecution(aiVerdictKey);
          const speculativeBuyWaiverApplied =
          ...
          const useRecalculatedCurrentEntry =
              ...
              livePrice != null &&
              currentEntryRequiredStopPrice != null;
          const contractEntryExecPrice = useAdaptiveCurrentEntry || useRecalculatedCurrentEntry ? livePrice : entryExecPriceShadow;
          const contractStopPrice = useRecalculatedCurrentEntry ? currentEntryRequiredStopPrice : mirroredStop;
          const contractEntryDistancePct = useAdaptiveCurrentEntry || useRecalculatedCurrentEntry ? 0 : entryDistancePctShadow;
          const contractStopDistancePct = useRecalculatedCurrentEntry
              ? currentEntryRequiredStopDistancePct
              : useAdaptiveCurrentEntry
                  ? currentPriceStopDistancePct
                  : stopDistancePct;
          const contractTargetDistancePct = useAdaptiveCurrentEntry || useRecalculatedCurrentEntry ? targetBufferFromCurrentPct : targetDistancePct;
          ...
          const contractExecutionReason: AlphaCandidate["executionReason"] =
              useAdaptiveCurrentEntry || useRecalculatedCurrentEntry ? 'VALID_EXEC' : executionReason;
          const chosenPlanType: AlphaCandidate["chosenPlanType"] =
              useRecalculatedCurrentEntry
                  ...
                  : decisionReason === 'wait_breakout_retest_required'
                      ? 'BREAKOUT'
                      : decisionReason === 'wait_current_rr_below_min' || decisionReason === 'wait_target_near_current' || decisionReason === 'wait_current_distance_above_adaptive' || decisionReason === 'wait_verdict_not_sidecar_actionable'
                          ? 'NO_TRADE'
                          : 'PULLBACK';
          ...
          return {
              mirroredEntry,
              mirroredTarget,
              mirroredStop: contractStopPrice,
              entryAnchorPrice,
              ...
              decisionReason,
              executionActionableVerdict,
              executionActionablePolicy: Array.from(STAGE6_ACTIONABLE_VERDICTS).join(','),
              executionActionableWaiver: speculativeBuyWaiverApplied,
              executionActionableWaiverReason: speculativeBuyWaiverApplied ? 'speculative_buy_explicit_waiver' : null,
              chosenPlanType,
              entryTactic,
              rrAtCurrentPrice,
              targetBufferFromCurrentPct,
              ...
              currentEntryStructureSupportReference,
              currentEntryStructureSupportGapAtr,
              currentEntryStructureNearestSupportLow,
              currentEntryStructureNearestSupportGapAtr,
              currentEntryStructureStopAlignedSupportLow,
              currentEntryStructureStopAlignedSupportGapAtr,
              currentEntryStructurePriceDriftPct,
              currentEntryStructureStopSupportRelation,
              currentEntryStructureSupportStopGapPct,
              currentEntryStructureStopAboveSupportPct,
              currentEntryStructureStopBelowSupportPct,
              ...
              breakoutRetestProofMaxBarsSinceRetest: breakoutRetestProof.maxBarsSinceRetest,
              ...
              breakoutRetestProofRetestTouchFound: breakoutRetestProof.retestTouchFound,
	              ...
	              breakoutRetestProofUndercutReclaimFound: breakoutRetestProof.undercutReclaimFound,
	              breakoutRetestProofUndercutExcessPct: breakoutRetestProof.undercutExcessPct,
	              breakoutRetestProofMaxUndercutExcessPct: breakoutRetestProof.maxUndercutExcessPct,
              ...
              breakoutRetestProofContinuationExtensionOk: breakoutRetestProof.continuationExtensionOk,
              ...
              targetRecalibrationRequiredTargetByExecutionFloorPrice: targetRecalibrationPolicy.requiredTargetByExecutionFloorPrice,
              targetRecalibrationRequiredTargetByExpectedReturnPrice: targetRecalibrationPolicy.requiredTargetByExpectedReturnPrice,
              ...
              targetRecalibrationExecutionFloorGapPct: targetRecalibrationPolicy.executionFloorGapPct,
              targetRecalibrationExpectedReturnGapPct: targetRecalibrationPolicy.expectedReturnGapPct,
              targetRecalibrationExecutionFloorShortfallPct: targetRecalibrationPolicy.executionFloorShortfallPct,
              targetRecalibrationExpectedReturnShortfallPct: targetRecalibrationPolicy.expectedReturnShortfallPct,
              targetRecalibrationExecutionFloorViable: targetRecalibrationPolicy.executionFloorViable,
              targetRecalibrationRequiredTargetSource: targetRecalibrationPolicy.requiredTargetSource,
              targetRecalibrationRequiredTargetDominantReason: targetRecalibrationPolicy.requiredTargetDominantReason,
              ...
              riskGeometryProofVerdict: riskGeometryPolicy.proofVerdict,
              riskGeometryRecalculatedStopPrice: riskGeometryPolicy.recalculatedStopPrice,
              ...
              riskGeometryRrAtRequiredTargetAndRecalculatedStop: riskGeometryPolicy.rrAtRequiredTargetAndRecalculatedStop,
              ...
              riskGeometryRequiredTargetDominantReason: riskGeometryPolicy.requiredTargetDominantReason,
              ...
              riskGeometryStopDistancePolicyVerdict: riskGeometryPolicy.stopDistancePolicyVerdict,
              riskGeometryTargetShortfallPolicyVerdict: riskGeometryPolicy.targetShortfallPolicyVerdict,
              ...
              riskGeometryTargetBufferAtRequiredTargetPct: riskGeometryPolicy.targetBufferAtRequiredTargetPct,
              riskGeometryTargetRecalibrationProofReady: riskGeometryPolicy.targetRecalibrationProofReady,
      ...
      const scoredCandidates: AlphaCandidate[] = finalData.map((item): AlphaCandidate => {
          ...
          return {
              ...
              executionActionableVerdict: executionContract.executionActionableVerdict,
              executionActionablePolicy: executionContract.executionActionablePolicy,
              executionActionableWaiver: executionContract.executionActionableWaiver,
              executionActionableWaiverReason: executionContract.executionActionableWaiverReason,
              ...
              currentEntryStructureSupportReference: executionContract.currentEntryStructureSupportReference,
              currentEntryStructureSupportGapAtr: executionContract.currentEntryStructureSupportGapAtr,
              currentEntryStructureNearestSupportLow: executionContract.currentEntryStructureNearestSupportLow,
              currentEntryStructureNearestSupportGapAtr: executionContract.currentEntryStructureNearestSupportGapAtr,
              currentEntryStructureStopAlignedSupportLow: executionContract.currentEntryStructureStopAlignedSupportLow,
              currentEntryStructureStopAlignedSupportGapAtr: executionContract.currentEntryStructureStopAlignedSupportGapAtr,
              currentEntryStructurePriceDriftPct: executionContract.currentEntryStructurePriceDriftPct,
              currentEntryStructureStopSupportRelation: executionContract.currentEntryStructureStopSupportRelation,
              currentEntryStructureSupportStopGapPct: executionContract.currentEntryStructureSupportStopGapPct,
              currentEntryStructureStopAboveSupportPct: executionContract.currentEntryStructureStopAboveSupportPct,
              currentEntryStructureStopBelowSupportPct: executionContract.currentEntryStructureStopBelowSupportPct,
              ...
              breakoutRetestProofReasons: executionContract.breakoutRetestProofReasons,
              breakoutRetestProofSource: executionContract.breakoutRetestProofSource,
              breakoutRetestProofLatestBarDate: executionContract.breakoutRetestProofLatestBarDate,
              breakoutRetestProofLatestClose: executionContract.breakoutRetestProofLatestClose,
              ...
              breakoutRetestProofMaxBarsSinceRetest: executionContract.breakoutRetestProofMaxBarsSinceRetest,
              ...
              breakoutRetestProofContinuationExtensionOk: executionContract.breakoutRetestProofContinuationExtensionOk,
              ...
              targetRecalibrationRequiredTargetByExpectedReturnPrice: executionContract.targetRecalibrationRequiredTargetByExpectedReturnPrice,
              ...
              riskGeometryProofVerdict: executionContract.riskGeometryProofVerdict,
              riskGeometryRecalculatedStopPrice: executionContract.riskGeometryRecalculatedStopPrice,
              ...
              sidecarConvictionFloor: executionContract.sidecarConvictionFloor,
              sidecarConvictionPass: executionContract.sidecarConvictionPass,
              sidecarConvictionMargin: executionContract.sidecarConvictionMargin,
              sidecarConvictionPolicyVerdict: executionContract.sidecarConvictionPolicyVerdict,
              sidecarConvictionReasons: executionContract.sidecarConvictionReasons,
              ...
              zeroExecutableFormulaSeverity: executionContract.zeroExecutableFormulaSeverity,
              zeroExecutableTargetShortfallPct: executionContract.zeroExecutableTargetShortfallPct,
              zeroExecutableRiskTargetShortfallPct: executionContract.zeroExecutableRiskTargetShortfallPct,
              zeroExecutableBreakoutProofGapCount: executionContract.zeroExecutableBreakoutProofGapCount,
              zeroExecutableStructureProofGapCount: executionContract.zeroExecutableStructureProofGapCount,
              zeroExecutableFormulaObservedValue: executionContract.zeroExecutableFormulaObservedValue,
              zeroExecutableFormulaThresholdValue: executionContract.zeroExecutableFormulaThresholdValue,
              zeroExecutableFormulaDeltaValue: executionContract.zeroExecutableFormulaDeltaValue,
              zeroExecutableFormulaUnit: executionContract.zeroExecutableFormulaUnit,
              zeroExecutableFormulaEvidenceBasis: executionContract.zeroExecutableFormulaEvidenceBasis,
              zeroExecutableFormulaAdjustmentKnob: executionContract.zeroExecutableFormulaAdjustmentKnob,
              zeroExecutableFormulaAdjustmentDirection: executionContract.zeroExecutableFormulaAdjustmentDirection,
              zeroExecutableFormulaAdjustmentMagnitude: executionContract.zeroExecutableFormulaAdjustmentMagnitude,
      ...
      addLog(
          `[HF_BLEND] enabled=${HF_BLEND_ENABLED} rawVite=${hfBlendEnabledRawVite || 'empty'} rawLegacy=${hfBlendEnabledRawLegacy || 'empty'} applied=${hfBlendAppliedCount}/${scoredCandidates.length} up=${hfBlendPositiveCount} down=${hfBlendNegativeCount} netDelta=${hfBlendNetDeltaExecution} weight=${HF_BLEND_WEIGHT} maxDelta=${HF_BLEND_MAX_DELTA} posScale=${HF_BLEND_POSITIVE_SCALE} negScale=${HF_BLEND_NEGATIVE_SCALE} minArticles=${HF_BLEND_MIN_ARTICLE_COUNT} maxNewsAgeH=${HF_BLEND_MAX_NEWS_AGE_HOURS} newsCountLow=${hfBlendNewsCountLow} newsStale=${hfBlendNewsStale} earningsWindowEnabled=${HF_EARNINGS_WINDOW_ENABLED} earningsBlockDays=${HF_EARNINGS_WINDOW_BLOCK_DAYS} earningsReduceDays=${HF_EARNINGS_WINDOW_REDUCE_DAYS} earningsReduceFactor=${HF_EARNINGS_WINDOW_REDUCE_FACTOR} earningsBlocked=${hfBlendEarningsBlocked} earningsReduced=${hfBlendEarningsReduced}`,
      ...
      const executablePool: AlphaCandidate[] = primaryPool.filter(
          item => item.executionBucket === 'EXECUTABLE' && item.executionActionableVerdict !== false
      );
      ...
      const nonActionableExecutableBlockedCount = primaryPool.filter(
          item => item.decisionReason === 'wait_verdict_not_sidecar_actionable'
      ).length;
      ...
      if (accessToken) {
          ...
          const top6AuditTrail = top6ArchiveCandidates.map(item => ({
              ...
              currentEntryStructureSupportReference: normalizeOptionalText(item.currentEntryStructureSupportReference),
              currentEntryStructureSupportGapAtr: toOptionalFiniteNumber(item.currentEntryStructureSupportGapAtr),
              currentEntryStructureNearestSupportLow: toOptionalFiniteNumber(item.currentEntryStructureNearestSupportLow),
              currentEntryStructureNearestSupportGapAtr: toOptionalFiniteNumber(item.currentEntryStructureNearestSupportGapAtr),
              currentEntryStructureStopAlignedSupportLow: toOptionalFiniteNumber(item.currentEntryStructureStopAlignedSupportLow),
              currentEntryStructureStopAlignedSupportGapAtr: toOptionalFiniteNumber(item.currentEntryStructureStopAlignedSupportGapAtr),
              currentEntryStructurePriceDriftPct: toOptionalFiniteNumber(item.currentEntryStructurePriceDriftPct),
              currentEntryStructureStopSupportRelation: normalizeOptionalText(item.currentEntryStructureStopSupportRelation),
              currentEntryStructureSupportStopGapPct: toOptionalFiniteNumber(item.currentEntryStructureSupportStopGapPct),
              currentEntryStructureStopAboveSupportPct: toOptionalFiniteNumber(item.currentEntryStructureStopAboveSupportPct),
              currentEntryStructureStopBelowSupportPct: toOptionalFiniteNumber(item.currentEntryStructureStopBelowSupportPct),
              ...
              breakoutRetestProofMaxBarsSinceRetest: toOptionalFiniteNumber(item.breakoutRetestProofMaxBarsSinceRetest),
              ...
              breakoutRetestProofContinuationExtensionOk: item.breakoutRetestProofContinuationExtensionOk == null ? null : Boolean(item.breakoutRetestProofContinuationExtensionOk),
              ...
              targetRecalibrationRequiredTargetByExecutionFloorPrice: toOptionalFiniteNumber(item.targetRecalibrationRequiredTargetByExecutionFloorPrice),
              targetRecalibrationRequiredTargetByExpectedReturnPrice: toOptionalFiniteNumber(item.targetRecalibrationRequiredTargetByExpectedReturnPrice),
              ...
              targetRecalibrationExecutionFloorGapPct: toOptionalFiniteNumber(item.targetRecalibrationExecutionFloorGapPct),
              targetRecalibrationExpectedReturnGapPct: toOptionalFiniteNumber(item.targetRecalibrationExpectedReturnGapPct),
              targetRecalibrationExecutionFloorShortfallPct: toOptionalFiniteNumber(item.targetRecalibrationExecutionFloorShortfallPct),
              targetRecalibrationExpectedReturnShortfallPct: toOptionalFiniteNumber(item.targetRecalibrationExpectedReturnShortfallPct),
              targetRecalibrationExecutionFloorViable: item.targetRecalibrationExecutionFloorViable == null ? null : Boolean(item.targetRecalibrationExecutionFloorViable),
              targetRecalibrationRequiredTargetSource: normalizeOptionalText(item.targetRecalibrationRequiredTargetSource),
              targetRecalibrationRequiredTargetDominantReason: normalizeOptionalText(item.targetRecalibrationRequiredTargetDominantReason),
              ...
              riskGeometryProofVerdict: normalizeOptionalText(item.riskGeometryProofVerdict),
              riskGeometryRecalculatedStopPrice: toOptionalFiniteNumber(item.riskGeometryRecalculatedStopPrice),
              ...
              riskGeometryRequiredTargetDominantReason: normalizeOptionalText(item.riskGeometryRequiredTargetDominantReason),
              ...
              riskGeometryStopDistancePolicyVerdict: normalizeOptionalText(item.riskGeometryStopDistancePolicyVerdict),
              riskGeometryTargetShortfallPolicyVerdict: normalizeOptionalText(item.riskGeometryTargetShortfallPolicyVerdict),
              ...
              zeroExecutableFormulaSeverity: toOptionalFiniteNumber(item.zeroExecutableFormulaSeverity),
              zeroExecutableTargetShortfallPct: toOptionalFiniteNumber(item.zeroExecutableTargetShortfallPct),
              zeroExecutableRiskTargetShortfallPct: toOptionalFiniteNumber(item.zeroExecutableRiskTargetShortfallPct),
              zeroExecutableBreakoutProofGapCount: toOptionalFiniteNumber(item.zeroExecutableBreakoutProofGapCount),
              zeroExecutableStructureProofGapCount: toOptionalFiniteNumber(item.zeroExecutableStructureProofGapCount),
              zeroExecutableFormulaObservedValue: toOptionalFiniteNumber(item.zeroExecutableFormulaObservedValue),
              zeroExecutableFormulaThresholdValue: toOptionalFiniteNumber(item.zeroExecutableFormulaThresholdValue),
              zeroExecutableFormulaDeltaValue: toOptionalFiniteNumber(item.zeroExecutableFormulaDeltaValue),
              zeroExecutableFormulaUnit: normalizeOptionalText(item.zeroExecutableFormulaUnit),
              zeroExecutableFormulaEvidenceBasis: normalizeOptionalText(item.zeroExecutableFormulaEvidenceBasis),
              zeroExecutableFormulaAdjustmentKnob: normalizeOptionalText(item.zeroExecutableFormulaAdjustmentKnob),
              zeroExecutableFormulaAdjustmentDirection: normalizeOptionalText(item.zeroExecutableFormulaAdjustmentDirection),
              zeroExecutableFormulaAdjustmentMagnitude: toOptionalFiniteNumber(item.zeroExecutableFormulaAdjustmentMagnitude),
              ...
              executionActionablePolicy: normalizeOptionalText(item.executionActionablePolicy),
              executionActionableWaiver: Boolean(item.executionActionableWaiver),
              executionActionableWaiverReason: normalizeOptionalText(item.executionActionableWaiverReason),
      ...
      const mapToNotionCandidate = (item: any): NotionSyncCandidate => ({
          ...
          executionVerdict: item?.executionVerdict,
          executionActionableVerdict: Boolean(item?.executionActionableVerdict),
          executionActionablePolicy: item?.executionActionablePolicy,
          executionActionableWaiver: Boolean(item?.executionActionableWaiver),
          executionActionableWaiverReason: item?.executionActionableWaiverReason,
