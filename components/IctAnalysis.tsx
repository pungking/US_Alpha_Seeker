...
const IctAnalysis: React.FC<Props> = ({ onComplete, autoStart }) => {
  ...
  const [progress, setProgress] = useState(() => {
    const cached = sessionStorage.getItem('stage5_progress');
    return cached ? JSON.parse(cached) : { current: 0, total: 0 };
  });
  const [logs, setLogs] = useState<string[]>(() => {
    const cached = sessionStorage.getItem('stage5_logs');
    return cached ? JSON.parse(cached) : ['> ICT_Node v5.5.0: Final Multi-Dimensional Sieve.'];
  });
  const [results, setResults] = useState<IctScoredTicker[]>(() => {
    const cached = sessionStorage.getItem('stage5_results');
  ...
  useEffect(() => {
    sessionStorage.setItem('stage5_logs', JSON.stringify(logs));
    sessionStorage.setItem('stage5_progress', JSON.stringify(progress));
    sessionStorage.setItem('stage5_results', JSON.stringify(results));
  ...
  const executeIntegratedIctProtocol = async () => {
    ...
    try {
      ...
      const tempResults: IctScoredTicker[] = [];
      ...
      const payload = {
        manifest: { version: "5.5.0", source: listRes.files[0].name, count: final50.length, timestamp: new Date().toISOString() },
    ...
    <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
      ...
      <div className="xl:col-span-1">
        <div className="glass-panel h-[720px] rounded-[40px] bg-slate-950 border-l-4 border-l-indigo-600 flex flex-col p-6 shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between mb-8 px-2"><h3 className="font-black text-white text-[10px] uppercase tracking-[0.4em] italic">ICT_Terminal</h3></div>
          <div ref={logRef} className="flex-1 bg-black/70 p-6 rounded-[32px] font-mono text-[9px] text-indigo-300/60 overflow-y-auto no-scrollbar space-y-4 border border-white/5 leading-relaxed">
            {logs.map((l, i) => (<div key={i} className={`pl-4 border-l-2 ${l.includes('[OK]') ? 'border-emerald-500 text-emerald-400' : 'border-indigo-900'}`}>{l}</div>))}
          </div>
        </div>
      </div>
    </div>
