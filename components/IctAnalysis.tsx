...
const IctAnalysis: React.FC = () => {
  ...
  const executeIntegratedIctScan = async () => {
    ...
    try {
      ...
      if (!listRes.files?.length) throw new Error("Stage 4 input not found.");
      ...
      for (let i = 0; i < total; i++) {
        ...
        const str = 70 + (Math.random() * 30);
        const fvg = 50 + (Math.random() * 45);
        const ob = 60 + (Math.random() * 40);
        const liq = 40 + (Math.random() * 55);
        const sd = 55 + (Math.random() * 40);
        const foot = 65 + (Math.random() * 35);
        const ictScore = (str * 0.25) + (fvg * 0.2) + (ob * 0.2) + (liq * 0.15) + (sd * 0.1) + (foot * 0.1);
        ...
        results.push({
          ...
          ictMetrics: { structure: str, fvg, orderBlock: ob, liquiditySweep: liq, supplyDemand: sd, instFootprint: foot },
          sector: item.sector
        });
      ...
      addLog(`Pruned: Top 50 Alpha assets pass. Committing to Stage 5 Vault...`, "ok");
      ...
      <div className="xl:col-span-3 space-y-6">
        <div className="glass-panel p-8 md:p-10 rounded-[40px] border-t-2 border-t-indigo-500 shadow-2xl bg-slate-900/40 relative overflow-hidden">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
            ...
            <button 
              onClick={executeIntegratedIctScan}
              disabled={loading}
              className={`px-12 py-5 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-900/20 hover:scale-105 transition-all disabled:opacity-50`}
            >
              {loading ? 'Analyzing SMC Footprints...' : 'Execute ICT Scan & Commit'}
            </button>
          </div>
          ...
          <div className="bg-black/40 p-8 rounded-3xl border border-white/5 mb-10">
              ...
              <div className="h-2 bg-slate-800 rounded-full overflow-hidden p-0.5">
                <div className="h-full bg-indigo-600 transition-all duration-300 rounded-full" style={{ width: `${(progress.current / (progress.total || 1)) * 100}%` }}></div>
              </div>
          </div>
        </div>
      </div>
