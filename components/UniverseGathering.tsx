...
const UniverseGathering: React.FC<Props> = ({ onAuthSuccess }) => {
  ...
  const [stats, setStats] = useState({
    found: 0,
    synced: 0,
    target: 5500, // 나스닥 + NYSE(S&P) 보통주 예상 합계
    elapsed: 0,
    remaining: 0,
  ...
  const [nodes, setNodes] = useState<NodeStatus[]>([
    { id: 'nodes_nasdaq', label: 'NASDAQ_Exchange_Node', count: 0, state: 'Idle' },
    { id: 'nodes_nyse', label: 'NYSE_S&P_Exchange_Node', count: 0, state: 'Idle' }
  ]);

  const [logs, setLogs] = useState<string[]>(['> Matrix Standby. Targeted: NASDAQ & S&P (NYSE) Equities.']);
  ...
  const runPipeline = async (token: string) => {
    ...
    addLog("Phase 1: Specialized Exchange Discovery Engaged (NASDAQ + NYSE/S&P)", "info");

    const capture = async (exchange: string, nodeId: string) => {
      updateNode(nodeId, 0, 'Scanning');
      // exchange=XNAS (NASDAQ), XNYS (NYSE) 필터 적용 및 보통주(CS)만 타겟팅
      let url = `https://api.polygon.io/v3/reference/tickers?exchange=${exchange}&type=CS&market=stocks&active=true&limit=1000&apiKey=${polygonKey}`;
      let localCount = 0;
      ...
      while (url && !stopReq.current) {
        try {
          ...
          if (res.results) {
            res.results.forEach((t: any) => {
              if (!registry.has(t.ticker)) {
                registry.set(t.ticker, { ...t, exchange_node: exchange, captured_at: new Date().toISOString() });
                localCount++;
              }
            });
          ...
          addLog(`Node [${exchange}] Error: API access restricted or network error.`, "err");
          break; 
        }
      }
      updateNode(nodeId, localCount, 'Success');
      addLog(`Node [${exchange}] Sync Complete: ${localCount} tickers indexed.`, "ok");
    };
    ...
    if (stopReq.current) {
      ...
      addLog("Gathering Protocol Aborted.", "err");
      return;
    }

    addLog(`Phase 1 Concluded. Total Unique Core Equities: ${registry.size}`, "ok");
    setStats(prev => ({ ...prev, phase: 'Commit' }));

    // Phase 2: Single Master File Construction
    addLog("Phase 2: Constructing Single Master Universe Vault...", "info");
    const masterData = Array.from(registry.values());
    const fileName = `STAGE0_MASTER_UNIVERSE_${new Date().toISOString().split('T')[0]}.json`;
    
    const payload = {
      manifest: {
        version: "1.7-EXCHANGE-CENTERED",
        total_count: masterData.length,
        exchanges: ["NASDAQ (XNAS)", "NYSE (XNYS)"],
        note: "Includes all S&P 500 and major NASDAQ constituents.",
    ...
    if (folderId) {
      ...
      if (success) {
        ...
        addLog(`Master File Saved: ${fileName}`, "ok");
      } else {
        addLog("Critical: Master Vault Write Failure.", "err");
      }
    }

    if (timerRef.current) clearInterval(timerRef.current);
    setIsEngineRunning(false);
    addLog("Universe Gathering Node Cycle Finished.", "ok");
  };
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <div className="glass-panel p-8 md:p-10 rounded-[40px] border-t-2 border-t-blue-500 shadow-2xl bg-slate-900/40 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
            <div className="flex items-center space-x-6">
              ...
              <div>
                <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">Omni_Nexus v1.7</h2>
                <div className="flex items-center mt-2 space-x-2">
                  <span className="text-[8px] px-2 py-0.5 bg-blue-500/10 text-blue-400 rounded-md font-black border border-blue-500/20 uppercase tracking-widest">Exchange_Centered</span>
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.2em]">NASDAQ & S&P Coverage</p>
                </div>
              </div>
            </div>
            <button 
            ...
            >
              {isEngineRunning ? 'Abort Pipeline' : 'Initiate Exchange Capture'}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            {[
              { label: 'NASDAQ+S&P Found', val: stats.found.toLocaleString(), color: 'text-white' },
              { label: 'Sync Pipeline', val: stats.phase === 'Finalized' ? 'COMPLETE' : stats.phase, color: 'text-blue-400' },
              { label: 'Uptime Cycle', val: formatTime(stats.elapsed), color: 'text-slate-400' },
              { label: 'Storage Node', val: 'MASTER_JSON', color: 'text-emerald-400' }
            ].map((s, i) => (
          ...
          <div className="space-y-6 mb-10">
            <div className="space-y-2">
              <div className="flex justify-between items-end px-2">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Discovery Progress (Core Exchange Filter)</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            {nodes.map(n => (
              <div key={n.id} className="bg-slate-900/60 p-6 rounded-3xl border border-white/5 flex flex-col items-center group hover:bg-slate-800/40 transition-all">
                <div className="flex items-center space-x-2 mb-2">
                  <div className={`w-1.5 h-1.5 rounded-full ${n.state === 'Scanning' ? 'bg-blue-500 animate-ping' : n.state === 'Success' ? 'bg-emerald-500' : 'bg-slate-700'}`}></div>
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">{n.label}</p>
                </div>
                <p className="text-xl font-mono font-black text-white">{n.count.toLocaleString()}</p>
                <p className={`mt-3 px-4 py-1.5 rounded-xl text-[7px] font-black uppercase ${n.state === 'Scanning' ? 'bg-blue-500/10 text-blue-400' : n.state === 'Success' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-800 text-slate-600'}`}>
                  {n.state}
                </p>
              </div>
            ))}
      ...
      <div className="xl:col-span-1">
        <div className="glass-panel h-[620px] rounded-[40px] bg-slate-950 border-l-4 border-l-blue-600 flex flex-col p-6 shadow-2xl relative">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-black text-white text-[10px] uppercase tracking-[0.4em] italic flex items-center">
              <span className="w-2 h-0.5 bg-blue-500 mr-3"></span>
              Exchange_Terminal
            </h3>
            <span className="text-[8px] font-mono text-slate-700">NODE_TX_0.1</span>
          </div>
          ...
          <div className="mt-8 space-y-3">
            <button onClick={() => window.open(`https://drive.google.com/drive/folders/${GOOGLE_DRIVE_TARGET.rootFolderId}`, '_blank')} className="w-full py-4 bg-white text-black rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all shadow-xl">Inspect Universe Vault</button>
            <div className="p-5 bg-blue-600/5 rounded-2xl border border-blue-500/10">
              <p className="text-[7px] text-blue-400 font-black uppercase tracking-[0.2em] mb-2">Targeting Logic</p>
              <p className="text-[9px] text-slate-400 font-bold italic leading-snug">Fetching XNAS/XNYS with type=CS ensures zero-loss coverage of S&P 500 and NASDAQ components.</p>
            </div>
          </div>
        </div>
      </div>
