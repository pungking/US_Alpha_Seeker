...
const UniverseGathering: React.FC<Props> = ({ onAuthSuccess }) => {
  ...
  const [stats, setStats] = useState({
    found: 0,
    synced: 0,
    target: 12000, // 전체 미국 시장 Active 티커 예상
    elapsed: 0,
    phase: 'Idle' as 'Idle' | 'Discovery' | 'Commit' | 'Finalized'
  });

  const [logs, setLogs] = useState<string[]>(['> Engine v1.9: Snapshot Protocol Ready.']);
  ...
  const startEngine = async () => {
      ...
      const client = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: (res: any) => {
          if (res.access_token) {
            ...
            onAuthSuccess?.(true);
            runSnapshotPipeline(res.access_token);
          }
        },
      });
    ...
    runSnapshotPipeline(token);
  };

  const runSnapshotPipeline = async (token: string) => {
    ...
    addLog("Initiating Universal Snapshot (Prices, Volume, Change %)...", "info");

    try {
      // Step 1: Market Snapshot (전 종목 실시간/마감 데이터 한 번에 수집)
      ...
      if (snapRes.tickers) {
        addLog(`Snapshot Retrieved: ${snapRes.tickers.length} candidates identified.`, "ok");
        
        snapRes.tickers.forEach((t: any) => {
          // 데이터 무결성 확인 및 저장
          newRegistry.set(t.ticker, {
            symbol: t.ticker,
            price: t.min?.c || t.prevDay?.c || 0,
            volume: t.day?.v || 0,
        ...
        addLog("Data Enrichment Complete. Symbol integrity verified.", "ok");
      } else {
        throw new Error("Invalid Snapshot Response");
      }

      setStats(prev => ({ ...prev, phase: 'Commit' }));
      addLog("Phase 2: Master Vault Synchronization...", "info");

      const masterData = Array.from(newRegistry.values());
      const fileName = `STAGE0_MASTER_UNIVERSE_v1.9.json`;
      
      const payload = {
        manifest: {
          version: "1.9-SNAPSHOT-ENRICHED",
          total_count: masterData.length,
          fields: ["symbol", "price", "volume", "change", "updated"],
      ...
      if (folderId) {
        ...
        if (success) {
          ...
          addLog(`Master Vault Saved Successfully.`, "ok");
        }
      }

    } catch (e) {
      addLog(`Critical Pipeline Failure: ${e}`, "err");
    } finally {
        ...
        <div className="glass-panel p-8 md:p-10 rounded-[40px] border-t-2 border-t-blue-500 shadow-2xl bg-slate-900/40 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
            <div className="flex items-center space-x-6">
              ...
              <div>
                <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">Omni_Nexus v1.9</h2>
                <div className="flex items-center mt-2 space-x-2">
                  <span className="text-[8px] px-2 py-0.5 bg-indigo-500/20 text-indigo-400 rounded-md font-black border border-indigo-500/20 uppercase tracking-widest">Snapshot_Protocol</span>
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.2em]">Full Market Coverage (Price/Vol/Change)</p>
                </div>
              </div>
            </div>
            <button 
            ...
            >
              {isEngineRunning ? 'Scanning Market...' : 'Engage Full Market Snapshot'}
            </button>
          </div>

          {/* Verification Search Bar (NEW) */}
              ...
              <div className={`flex-1 flex items-center px-6 rounded-xl border transition-all ${searchResult ? 'bg-emerald-500/5 border-emerald-500/20 text-emerald-400' : 'bg-slate-900 border-white/5 text-slate-600'}`}>
                {searchResult ? (
                  <div className="flex justify-between w-full font-mono text-[10px] font-bold">
                    <span>{searchResult.symbol} FOUND</span>
                    <span>Last: ${searchResult.price}</span>
                    <span>Vol: {searchResult.volume.toLocaleString()}</span>
                  </div>
                ) : (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            {[
              { label: 'Market Tickers', val: stats.found.toLocaleString(), color: 'text-white' },
              { label: 'Data Quality', val: 'ENRICHED', color: 'text-emerald-400' },
              { label: 'Cycle Time', val: `${stats.elapsed}s`, color: 'text-slate-400' },
              { label: 'Target Vault', val: 'V1.9_MASTER', color: 'text-blue-400' }
            ].map((s, i) => (
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-end px-2">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Universe Construction Progress</p>
              <p className="text-[10px] font-mono font-bold text-white">{stats.phase === 'Finalized' ? '100%' : `${Math.min(100, (stats.found / stats.target) * 100).toFixed(1)}%`}</p>
            </div>
      ...
      <div className="xl:col-span-1">
        <div className="glass-panel h-[680px] rounded-[40px] bg-slate-950 border-l-4 border-l-blue-600 flex flex-col p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-black text-white text-[10px] uppercase tracking-[0.4em] italic">Snapshot_Terminal</h3>
          </div>
          ...
          <div className="mt-8 space-y-3 text-center">
            <div className="p-5 bg-blue-600/5 rounded-2xl border border-blue-500/10">
              <p className="text-[7px] text-blue-400 font-black uppercase tracking-[0.2em] mb-2">V1.9 Upgrade Logic</p>
              <p className="text-[9px] text-slate-400 font-bold italic leading-snug">All 10,000+ active tickers are captured with price and volume data in a single high-speed cycle.</p>
            </div>
          </div>
        </div>
      </div>
