...
const UniverseGathering: React.FC<Props> = ({ onAuthSuccess, isActive, apiStatuses, onStockSelected, autoStart, onComplete }) => {
  ...
  // [FIX] State for dynamic styling in ticker box
  const [priceFlash] = useState<'up' | 'down' | null>(null);
  const isLive = true; // Treat as live for styling purposes (green/red colors)
                    ...
                    {searchResult ? (
                      <div className="flex justify-between items-center w-full font-mono text-[10px] font-bold">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`text-[8px] font-black ${Math.abs(searchResult.change) < 0.01 ? 'text-slate-400' : searchResult.change > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
