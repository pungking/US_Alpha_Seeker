...
const UniverseGathering: React.FC<Props> = ({ onAuthSuccess, isActive, apiStatuses, onStockSelected, autoStart, onComplete }) => {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
      {/* CONFIG MODAL - Fixed로 변경하여 z-index 이슈 해결 */}
      {showConfig && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-2xl p-6">
          <div className="glass-panel w-full max-w-md p-10 rounded-[40px] border border-blue-500/30 shadow-[0_0_50px_rgba(59,130,246,0.2)]">
            <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter mb-6">Vault Configuration</h3>
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Google Drive Client ID</label>
                <input 
                  type="text" 
                  value={clientId}
                  onChange={(e) => setClientId(e.target.value)}
                  placeholder="Enter Client ID (from GCP Console)..."
                  className="w-full bg-black/60 border border-white/10 rounded-2xl px-5 py-4 text-white font-mono text-xs focus:border-blue-500 outline-none transition-all"
                />
              </div>
              <div className="flex gap-3 pt-4">
                <button onClick={handleSaveConfig} className="flex-1 py-4 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-900/20 active:scale-95 transition-all">Save Identity</button>
                <button onClick={() => setShowConfig(false)} className="px-6 py-4 bg-slate-800 text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-700 transition-all">Cancel</button>
              </div>
              <p className="text-[9px] text-slate-600 text-center leading-relaxed mt-4">
                * Client ID가 없는 경우 GCP 콘솔에서 OAuth 2.0 클라이언트 ID를 생성해야 합니다.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="xl:col-span-3 space-y-6">
        <div className="glass-panel p-5 md:p-8 lg:p-10 rounded-[32px] border-t-2 border-t-blue-500 shadow-2xl relative">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-6">
              ...
              <div>
                ...
                <div className="flex items-center mt-2 space-x-2">
                  ...
                  <button 
                    ...
                    className="text-[8px] px-2 py-0.5 bg-slate-800 text-white rounded-md font-black border border-blue-500/50 uppercase hover:bg-blue-600 transition-all active:scale-90"
                  >
                    ⚙ CONFIG
                  </button>
                </div>
            ...
            <button 
              onClick={startEngine} 
              className={`px-12 py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${!accessToken ? 'bg-amber-600 text-white animate-pulse shadow-amber-900/20' : 'bg-blue-600 text-white shadow-xl hover:scale-105'}`}
            >
          ...
          <div className="bg-black/40 p-6 rounded-3xl border border-white/5 mb-8">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <input 
                ...
                className="flex-1 bg-slate-950 border border-white/10 rounded-xl px-6 py-4 text-white font-mono text-sm uppercase outline-none focus:border-blue-500/50"
                value={searchTerm}
