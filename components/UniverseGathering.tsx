...
// [HELPER] Smart Ratio Normalizer (v2.1)
// Automatically detects if a value is decimal (0.15) or percent (15.0) and scales it to percent.
const normalizeRatio = (val: any, fieldName: string = ''): number => {
    if (val === null || val === undefined || val === '') return 0;
    let num = Number(val);
    if (isNaN(num) || !isFinite(num)) return 0;

    // Fields that are NOT percentages (Multiples, Prices, Counts)
    const nonPercentFields = ['pe', 'eps', 'price', 'volume', 'marketCap', 'beta', 'pb', 'pbr', 'psr'];
    if (nonPercentFields.some(f => fieldName.toLowerCase().includes(f))) {
        return parseFloat(num.toFixed(2));
    }

    // Heuristic: If value is small (< 5.0 magnitude) for a Ratio like ROE/Margins, assume decimal representation of %
    // Exception: Dividend Yield can be naturally small (e.g. 2.3%), but if < 0.5 (0.5%), it might be decimal.
    // Safest bet: If abs < 2.0 (200% or 2.0x), scale up.
...
const UniverseGathering: React.FC<Props> = ({ onAuthSuccess, isActive, apiStatuses, onStockSelected, autoStart, onComplete }) => {
  ...
  const [logs, setLogs] = useState<string[]>(['> Engine v3.6.0: Triple Hybrid Fusion Mode (Smart Scaling Enabled).']);
  ...
  const executeTVScanner = async (): Promise<MasterTicker[]> => {
      ...
      return data.map((item: any) => ({
          ...
          pe: item.pe, 
          roe: normalizeRatio(item.roe, 'roe'), // Normalized!
          eps: item.eps,
          debtToEquity: normalizeRatio(item.debtToEquity, 'debt'), // Normalized!
          pbr: item.pbr || item.price_book_ratio_fq,
  ...
  const fuseDatasets = (tv: MasterTicker[], fmp: MasterTicker[], poly: MasterTicker[]): MasterTicker[] => {
      ...
      tv.forEach(item => {
          // Normalize ratios again just in case
          const enriched = {
              ...item,
              roe: normalizeRatio(item.roe, 'roe'),
              debtToEquity: normalizeRatio(item.debtToEquity, 'debt')
          };
          map.set(item.symbol, enriched);
      });
      ...
      poly.forEach(item => {
          if (map.has(item.symbol)) {
              const existing = map.get(item.symbol)!;
              // If Polygon date is newer than TV, update price/vol
  ...
  const runTripleFusionPipeline = async (token: string) => {
    ...
    try {
        ...
        const fileName = `STAGE0_MASTER_UNIVERSE_v3.6.0.json`;
        const payload = { 
            manifest: { 
                version: "3.6.0", 
                ...
                note: "Smart Ratio Scaling Applied"
            }, 
        ...
        <div className="glass-panel p-5 md:p-8 lg:p-10 rounded-[32px] md:rounded-[40px] border-t-2 border-t-blue-500 shadow-2xl bg-slate-900/40 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 md:mb-10 gap-6">
            <div className="flex items-center space-x-4 md:space-x-6">
              ...
              <div>
                <h2 className="text-xl md:text-3xl font-black text-white italic tracking-tighter uppercase leading-none">Omni_Nexus v3.6.0</h2>
