...
const App: React.FC = () => {
  ...
  const runAiAnalysis = async (symbolsToUse?: string[]) => {
    ...
    try {
      ...
      setAuditReports(prev => ({ ...prev, [auditBrain]: `### AUDIT_NODE_ERROR\n> ${err.message}` }));
    } finally {
        ...
        <div className="bg-black/40 rounded-[40px] border border-white/5 p-8 md:p-12 min-h-[300px] shadow-inner relative group">
          {isAiLoading ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center space-y-6">
              ...
              <p className="text-[10px] font-black text-emerald-500/60 uppercase tracking-[0.4em] animate-pulse">Synthesizing Strategic Audit...</p>
            </div>
          ) : auditReports[auditBrain] ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 opacity-30 text-center space-y-4">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.6em] italic">Awaiting Alpha Protocol Signal...</p>
            </div>
          )}
        </div>
      </section>
