...
const App: React.FC = () => {
  ...
  const runAiAnalysis = async (symbolsToUse?: string[]) => {
    ...
    try {
      ...
      setAuditReports(prev => ({ ...prev, [auditBrain]: `### AUDIT_ERROR\n> ${err.message}` }));
    } finally {
  ...
  const handleStageComplete = useCallback((symbols?: string[]) => {
    if (isAutoPilotActive) {
      if (currentStage < 6) {
        console.log(`[Auto-Pilot] Stage ${currentStage} Complete. Transitioning in 4s...`);
        setTimeout(() => {
          setCurrentStage(prev => prev + 1);
        }, 4000);
      } else {
        console.log("[Auto-Pilot] Full Cycle Successful.");
  ...
  return (
    <div className="min-h-screen pb-10 p-3 md:p-6 space-y-6 max-w-[1600px] mx-auto overflow-x-hidden">
      <div className="flex items-center glass-panel px-4 py-2.5 rounded-xl border-white/5 text-[8px] md:text-[9px] font-black uppercase tracking-widest text-slate-500 overflow-x-auto no-scrollbar whitespace-nowrap relative">
        ...
        <div className="ml-auto flex items-center space-x-4 shrink-0">
          ...
          >
            ...
            <span>{isAutoPilotActive ? 'Nexus_AutoPilot_Active' : 'Execute_Full_AutoPilot'}</span>
          </button>
      ...
      {/* 컴포넌트 데이터 보존을 위해 Conditional Rendering 사용 */}
        ...
        <div className="bg-black/40 rounded-[40px] border border-white/5 p-8 md:p-12 min-h-[300px] shadow-inner relative group">
          {isAiLoading ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center space-y-6">
              ...
              <p className="text-[10px] font-black text-emerald-500/60 uppercase tracking-[0.4em] animate-pulse">Analyzing Market Dynamics...</p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-24 opacity-30 text-center">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.6em] italic">Awaiting Protocol Signal...</p>
            </div>
          )}
        </div>
      </section>
